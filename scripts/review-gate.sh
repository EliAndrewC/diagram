...
for spec in $(printf '%s\n' "$changed" | grep -E '^specs/[^/]+/spec\.md$' || true); do
  [ -f "$spec" ] || continue
  # A VERDICT, not a MENTION (feature 154, 2026-08-29). The check used to be a bare `grep FAITHFUL`,
  ...
  # repository when it was written - 56 of 56 unchanged, and it is the guard's own "match INVOCATIONS
