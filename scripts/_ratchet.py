...
RATCHETS = {
    ...
    "done": Ratchet(
        target="done",
        baseline=385,
        reason="GUARD_EDIT_OK: RE-PINNED 2026-09-06 to 385 s, DOING WHAT THE PREVIOUS REASON ASKED. "
               "That reason (kept in full below) pinned 310 as provisional and said so twice: both of "
               "its candidate measurements shared the box with foreground runs from the same session, "
               "and it instructed 'RE-PIN FROM AN IDLE RUN when one exists'. It also predicted this "
               "firing - 'the bar will not bite until [the cheap pre-174 runs] age out'. They have. "
               "TWO THINGS ESTABLISH THE NEW NUMBER, and neither is a preference. (1) The 310 was "
               "pinned on 2026-08-31, and on 2026-09-02 the GM widened the measured coverage surface "
               "to the whole engine (`source = [\"l7r\"]`, +19 files / +1,843 statements) - so the old "
               "baseline predates a change that legitimately and permanently made `done` slower, and "
               "was never measuring today's target. (2) 385 s is the MEDIAN OF THE NINE recent green "
               "reference-scope runs that had no other recorded run overlapping them - the idle "
               "measurement the old reason wanted, taken from 25 recorded runs rather than the 2 it "
               "had. The overall median including contended runs is 399 s; 385 is pinned as the "
               "TIGHTER of the two, so this re-pin does not buy more slack than the evidence supports. "
               "TWO HYPOTHESES WERE TESTED AND BOTH REFUTED before concluding the level is legitimate, "
               "which is stated so nobody re-runs them: tooling-hash invalidation (runs WITH the whole "
               "tooling suite re-running median 399 s, WITHOUT 562 s - the wrong way round), and "
               "concurrency (385 s solo vs 399 s with something else running - no real effect). "
               "NOT EXPLAINED, and left on the record rather than smoothed away: one 806 s run on "
               "2026-09-06 (d4cf5c12), the slowest ever recorded, with nothing else running. It is "
               "the run that tipped the median from 399 (under the old ceiling) to 429 (over). If "
               "runs like it recur, this baseline is wrong and the cause is still unfound. "
               "NOTE FOR WHOEVER READS THIS NEXT: the old reason's suggested instrument, "
               "`make idle-tests`, no longer arms itself - the GM disabled idle-test ARMING on "
               "2026-09-06. The target still runs BY HAND, which is why it was disabled rather than "
               "deleted, so an idle measurement is still available; it just has to be asked for. "
