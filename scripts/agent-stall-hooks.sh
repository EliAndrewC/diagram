...
#   pending <subagents-dir> one bare agent id per line for every agent still awaiting a reply (feature 147's
...
try: print(json.load(sys.stdin).get("transcript_path",""))
  ...
  pending)
    ...
    # (last record a tool_result), stalled or not. Feature 147's pairing guard asks whether a
