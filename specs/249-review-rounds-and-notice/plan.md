...
- **P1 `batching-hooks.sh`**: the notice condition loses `is_recon_call` AND the `BG -eq 0` test; the
  block condition keeps both. Suite case 2c: two serial turns, then a Bash payload whose command contains
...
- **P2 `review-round-hooks.sh pretool`** (bash + one python block, the `escalation-hooks.sh` shape):
  ...
  `additionalContext` naming the round; refresh the snapshot; the round stored is round+1, or 1 when the
  recovered previous verdict is FAITHFUL (a new pass - an amendment); log `rewrote/mode-3-preamble`.
...
- **P4 the suite**: a fixture repo with `specs/301-fixture/spec.md`, a fake subagents dir with one
  transcript whose first line names the dir and whose last carries `CHANGES REQUIRED` (and a second
  whose last carries `FAITHFUL`, for the pass restart), payloads built with `transcript_path` pointing at
  the fixture; the cases FR-006 lists, the MODE 4 and MODE 1 pass-throughs included; the guard log
  isolated as every recording suite does.
- **P5 the census**: rows `("review-round", spec-fidelity dispatch naming no feature, "permitted",
  "no-feature")`, a MODE 4 dispatch `permitted`/`other-mode`, and the escape cases; `REVIEW_ROUND_OK` classified `command` with the agent-prompt
