...
**THE DIAGNOSIS ROUNDS 8-11 CONVERGED ON, kept because it is the useful part.** Four rounds found the
same mechanical failure in five different files: an item settled in the spec while a sentence asserting
...
for the GM rather than built unasked: *a gate check that what the operative docs SAY about the
mechanism matches what the config actually sets* - the count quoted in `CLAUDE.md` / `SKILL.md` /
`pyproject.toml` against the measured one, and any `fail_under` they name against what is really
configured. The same shape as `make notes-census`, which exists because a stated count and a drawn
count drifted apart three times. Round 11 noted that a count-only check would have caught neither of
its own two items, both of which were claims about a MECHANISM.
