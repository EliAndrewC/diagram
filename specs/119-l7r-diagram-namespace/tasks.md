...
- [x] T001 Take the Principle XIII gate baseline on unmodified code in a DETACHED WORKTREE
...
- [x] T002 Take the pool byte-identity baseline in the same worktree: regenerate every generator
...
- [x] T003 Confirm the two baselines are complete and readable, and that `/tmp/119-base` is a
...
- [x] T004 [US1] Write `webapp/tests/test_namespace_portion.py` asserting `l7r.__file__ is None`
...
- [x] T005 [US1] Delete `webapp/l7r/__init__.py` and move its docstring content ("Importing this
...
- [x] T006 [P] [US1] Repoint the bare-import side-effect consumers to `import l7r.app`:
...
- [x] T007 [P] [US1] Repoint the launch paths: `webapp/Dockerfile` CMD to
...
- [x] T008 [US1] Verify no OTHER `l7r` reference depends on the deleted `__init__.py`: grep
...
- [x] T009 [US1] Run the webapp gate: `cd webapp && make done`. Compare failures against T001's
      baseline; zero NEW failures is the bar.
- [x] T010 [US1] Smoke check: start the app the way the Dockerfile does
...
- [x] T011 [US2] Write `.claude/skills/diagram/tests/test_namespace_portion.py` asserting
...
- [x] T012 [US2] Create `.claude/skills/diagram/l7r/diagram/__init__.py` and `git mv` the eight
...
- [x] T013 [US2] Repath all 245 absolute engine imports per
...
- [x] T014 [US2] Repath the gate configuration in `.claude/skills/diagram/pyproject.toml` in all
...
- [x] T015 [US2] Repath the 26 generator import lines under `pool/` and `wip/` - ONE line each.
...
- [x] T016 [US2] Run the diagram gate: `cd .claude/skills/diagram && make done` (background it;
...
- [x] T017 [US2] Run the byte-identity sweep: regenerate every generator with `--frozen-ok` in a
...
- [x] T018 [P] [US2] Update the 39 live-doc command references across 13 files:
...
- [x] T019 [P] [US2] Update the structural prose that states WHERE engine code lives: the "Where
...
- [x] T020 [US2] Verify the new CLIs actually run:
...
- [x] T021 [US3] Write a test asserting the import direction - nothing under
...
- [x] T022 [US3] Create `l7r/diagram/sitegen/` with `types.py` (`Pt`, `Poly`, `SQ_FT_PER_ACRE`),
...
- [x] T023 [US3] Repoint `hamletgen`'s intra-package importers of the moved names:
...
- [x] T024 [US3] Add `l7r/diagram/sitegen/CLAUDE.md` whose FIRST lines state the membership rule
...
- [x] T025 [US3] Add the three `sitegen` modules to `[tool.mypy] files` and
...
- [x] T026 [US3] Record the standing rules where a village-tier session will actually read them:
...
- [x] T027 [US3] Run the diagram gate and the byte-identity sweep again, comparing against the
...
- [x] T028 Prove zero stale module paths repo-wide: grep `.py` for bare engine imports and live
      `.md` for old `python3 -m` forms; both must return nothing outside `specs/`.
- [x] T029 Re-run BOTH gates end to end (`webapp make done`, diagram `make done`) and compare to
...
- [x] T030 Confirm all three guard tests were observed RED and are now green, and that each names
