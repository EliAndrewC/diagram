...
- [ ] T02 `_hookmatch.py` gains `as_make_target(cmd)`: a bare pytest of ONE test file becomes
...
- [ ] T03 `_hookmatch.py` gains `bracket_pattern(cmd)`: a literal `pgrep -f` / `pkill -f` becomes the
...
- [ ] T04 `make-only`: rewrite a one-file bare pytest; keep the refusal for every other shape. FIX
...
- [ ] T05 `no-poll`: ask `_hookmatch.py` whether the shape is INVOKED (it refused the command writing
      ...
      the rewrite lands
- [ ] T06 `measure-hooks`: the same matcher change, and replace the stale limitation comment. The
...
- [ ] T07 `pair`: rewrite `make done` to `make verify` when a review is owed, with the line telling
...
- [ ] T08 `house-style`: correct the payload instead of refusing. The GM-writing exemption is
...
- [ ] T09 `guard-file`: return the `GUARD_EDIT_OK` line as context on a READ of a guard file; the
...
- [ ] T10 `batching`: attach the playbook one turn below the threshold. The window, the backoff and
...
- [ ] T11 every conversion calls `guard_log` (`rewrote` / `reminded` / `blocked`), and every suite
