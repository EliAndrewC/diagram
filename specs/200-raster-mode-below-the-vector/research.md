...
## R6. The implementation, measured over the pool (T06)

Every live pool page as regenerated by the gate (the clone, two modes) against main's page at
`5a396225` (feature 199, vector only), same machine, same run, Chromium headless, 1400 x 1000.
Raster CPU in ms per action (trace) at the opening view; the sweep is the mean wall ms per real pointer
move; the pixel column is the raster opening view against the vector one, pixels differing by more than
8/255 in 1,400,000:

| page | load old/new (s) | first frame new | raster ready | hover scrub | hover off | farmhouse | wheel | zoom | wheel 2x | sweep old/new | MB old/new | px diff |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| inashiro | 0.30 / 0.38 | 0.41 s | 363 ms | 208 / 29 | 109 / 10 | 21 / 5 | 186 / 26 | 65 / 25 | 61 / 25 | 17.7 / 16.8 | 9.1 / 7.7 | 73,062 |
| kuwabata | 0.36 / 0.38 | 0.41 s | 366 ms | 217 / 36 | 124 / 9 | 52 / 6 | 196 / 11 | 63 / 11 | 64 / 10 | 17.5 / 16.9 | 9.9 / 6.1 | 54,281 |
| kashikawa | 0.38 / 0.33 | 0.35 s | 311 ms | 129 / 16 | 162 / 10 | 0 / 1 | 264 / 13 | 96 / 12 | 181 / 12 | 17.9 / 16.8 | 12.6 / 6.6 | 68,986 |
| mizuguchi | 0.25 / 0.35 | 0.37 s | 320 ms | 236 / 25 | 113 / 10 | 0 / 1 | 128 / 18 | 94 / 15 | 167 / 14 | 18.0 / 16.8 | 7.4 / 7.4 | 67,205 |
| sawada | 0.40 / 0.31 | 0.32 s | 292 ms | 240 / 33 | 112 / 8 | 21 / 4 | 150 / 28 | 73 / 27 | 149 / 30 | 18.3 / 17.1 | 13.6 / 6.0 | 109,495 |

("load" is the load event; "first frame" is that plus two animation frames, the instant the vector
picture is on screen; "raster ready" is `l7rMap.readyAt()`, ms after navigation, when both images had
decoded and the page switched. "wheel 2x" is a wheel turn after one zoom step - still raster mode at
DPR 1 on every page.)

**SC-001** (hover the scrub under 60, hover off under 20, a farmhouse under 15, a wheel turn under 25,
a zoom step under 25): the first three hold on every page with room (worst 36 / 10 / 6). The wheel and
zoom criteria hold on Kuwabata, Kashikawa and Mizuguchi and are MISSED by 1-3 ms on Inashiro (26 / 25)
and Sawada (28 / 27) - the two square-or-wide maps, whose opening view shows the most map; the numbers
were set from Kuwabata's prototype (12-14 / 10-12) and those two pages have 5-10x less work than before
(186 -> 26, 150 -> 28; 65 -> 25, 73 -> 27). Reported, not moved.
**SC-002** holds: every page smaller (Mizuguchi equal at 7.4 MB - its scatter was mostly on the map).
**SC-003**: the load event is within +0.10 s of today's on three pages (Inashiro +0.08, Kuwabata +0.02,
Mizuguchi +0.10) and earlier on two (Kashikawa -0.05, Sawada -0.09); the same page's load moved 0.33-0.38
between runs during this feature, so this is the noise floor and not a clean pass of "no later than
today's" - the honest reading is "the same, to within a tenth of a second". Raster mode is ready 292-366
ms after navigation on every page, well inside the 0.8 s.
**The picture** (FR-005): 4-8% of pixels differ by more than 8/255 - anti-aliasing at every edge, resvg's
against Chromium's; crops compared by eye are the same picture (the prototype's R4 crops).

The guards' own readings from the gate's browser run on Kuwabata: raster mode ready 366 ms after
navigation; the id map agreed with the DOM on 98.2% of 2,100 points; hover the scrub 36 ms of raster
CPU and one wheel turn 11 against caps of 100 and 60.
