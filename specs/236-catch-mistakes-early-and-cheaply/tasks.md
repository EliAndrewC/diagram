...
- [ ] T01 `scripts/_patch.py`: apply (anchor, replacement) edits to a file, EACH EDIT ITS OWN WRITE.
      ...
      research: rendering
      verify: one `make quick` after the whole diff is re-read for convention misses
- [ ] T02 Anchors match WHITESPACE-INSENSITIVELY - four of six recorded misses spanned a line wrap
      ...
      research: rendering
- [ ] T03 `tests/tooling/test_patch.py`: the SC-001 case (three edits, the second anchor missing,
      ...
      research: rendering
- [ ] T04 Root `CLAUDE.md` names `_patch.py` beside the "Edit files with `Edit`" rule, as the
...
- [ ] T05 `scripts/_hm_shell.py` - the pure detectors, one leaf so guard suites depend on what they
      ...
      a `-F <file>` read.
      research: rendering
- [ ] T06 `scripts/shell-check-hooks.sh` (PreToolUse, Bash) runs the four detectors and REFUSES with
      ...
      research: rendering
- [ ] T07 Wire it in `.claude/settings.json` beside the other Bash guards.
      research: rendering
- [ ] T08 `scripts/fixtures/bash-parse-corpus-2026-09.json`: the 238 commands of `research.md` R6,
...
- [ ] T09 `scripts/test-shell-check-hooks.sh` + its table in `scripts/test_hooks_cases.py`: the
      ...
      research: rendering
- [ ] T10 `tests/tooling/test_guard_firing_log.py`: classify `SHELL_CHECK_OK` in the derived escape
...
- [ ] T11 `scripts/house-style-hooks.sh` sees Bash: the payload is scanned and the session is TOLD
      ...
      research: rendering
- [ ] T12 `scripts/check-house-style-delta.py`: scan the DELTA against the merge base plus every
      ...
      research: rendering
- [ ] T13 Exemptions judged from the WHOLE FILE, never the changed line - a line inside a multi-line
      ...
      research: rendering
- [ ] T14 It is a `make quick` PHASE, not a pytest test: testmon selects by changed code, and a scan
      ...
      research: rendering
- [ ] T15 `.specify/templates/tasks-template.md` carries the "American spellings, hyphens only"
      checklist line the GM asked for in advance (FR-009a).
      research: rendering

## Phase 4 - `spec-lint` (item 5: FR-010 to FR-011)

- [ ] T16 `scripts/spec-lint.py` with `--selftest`: the four checks of FR-010, failing with file and
      ...
      `tasks.md`.
      research: rendering
- [ ] T17 Checks 1, 3 and 4 apply only to a spec that HAS a `tasks.md`, so the number claim and the
      ...
      research: rendering
- [ ] T18 THIS spec passes checks 1 and 3 - run it against `specs/236-*/` and fix the spec, not the
      rule, if it does not (FR-010b).
      research: rendering
- [ ] T19 Wired at the gate (skill Makefile `static`, beside `check-file-scale.py`) and at push
...
- [ ] T20 `.claude/agents/spec-fidelity.md` documents MODE 3, VERIFY: confirm each item a round
      ...
      research: rendering
- [ ] T21 `.specify/templates/tasks-template.md` carries the review-task SHAPE - round, items,
      changed passages, mode - because the GM ruled doctrine alone will not hold it (FR-013).
      research: rendering
- [ ] T22 Root `CLAUDE.md`: the guard-table rows for the shell guard, the delta check and
...
- [ ] T23 Every check is proven to FIRE by removing its mechanism and watching a test go red
