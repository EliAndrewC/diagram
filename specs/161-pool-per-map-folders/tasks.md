...
- [x] T09 `pipeline/render_cache.py` - `regen_pool` asks for bundles (BOTH trees; the missing-exhibit
      warning follows the exhibits into the legacy tree). Reporting paths become tree-relative
      research: rendering
- [x] T10 `pipeline/pool_index.py` - discovery via the surface; `SKIP_DIRS`' non-map exclusion moves
      into the surface so the two cannot disagree
      research: rendering
- [x] T11 `tools/cache_audit.py` (live pool, `scripted` only), `tools/timings.py` (both trees, a
      ...
      research: rendering
- [x] T12 `tests/test_villages.py` - GENERATORS (live, scripted), the ratchet (BOTH trees, FR-013a),
      and the stale-render sweep (BOTH trees, FR-013b - the regression T02 found)
      research: rendering
- [x] T13 `tests/test_notes_census.py`, `tests/pipeline/test_pool_index.py`,
      `tests/tooling/pipeline/test_gencache.py` (it names `hoshizora`, a frozen map that changes
      trees), `tests/test_compound.py`
      research: rendering
- [x] T14 **the top risk (R4)**: add `legacy-hand-authored-pool` to the directory-prune tuples in
      ...
      merge ROUTE
      research: rendering
- [x] T15 `check_village/__main__.py`'s default manifest RE-POINTED at the live reference hamlet -
      ...
      research: rendering
- [x] T16 `ci/delta.py` `_ENGINE_DIRS` gains the legacy tree, keeping the merge-route classification
      ...
      research: rendering
- [x] T17 `scripts/review-gate.sh`'s changed-manifest pattern reaches both trees, and
      ...
      research: rendering
- [x] T18 the remaining guard-script fixtures that construct pool paths -
      `scripts/test-sync-with-main.sh`, `scripts/test-make-only-hooks.sh`
      research: rendering
- [x] T19 `Makefile`: the `GEN=`/`M=`/`A=`/`B=` defaults, `notes-census`' `$(wildcard)`, `verify`'s
      `git diff` glob, and `pool-index-if-stale`'s `find` - which must now watch BOTH trees, since the
      index covers both
      research: rendering
- [x] T20 **`pyproject.toml`'s `[tool.ruff] extend-exclude`** - round 2's highest-value find, and the
      ...
      research: rendering
- [x] T21 `make quick` green. Still the OLD tree - this is the checkpoint that proves Phase 2 changed
      behavior nowhere
      research: rendering

## Phase 3 - move ONE map (the reference-first step, constitution VI)

- [x] T22 `git mv` the inashiro bundle into `pool/hamlets/inashiro/`; deepen its own `sys.path`
      arithmetic by one level
      research: rendering
- [x] T23 prove it on the one map: `make map GEN=pool/hamlets/inashiro/inashiro.gen.py` green, the
      index rebuilt, its links resolving, and inashiro's moved files content-hash identical
      research: rendering

## Phase 4 - move the remaining 27

- [x] T24 the 4 other scripted hamlets and the 5 magistracies into per-map folders under `pool/`
      research: rendering
- [x] T25 create `legacy-hand-authored-pool/` and `git mv` all 18 frozen maps into
      `<tier>/<stem>/`, preserving the committed `.svg`/`.png` exhibits as RENAMES (FR-008)
      research: rendering
- [x] T26 deepen the `sys.path` arithmetic in all 27 remaining gens - the 18 frozen ones included. A
      frozen gen that is silently broken is worse than one that is merely frozen
      research: rendering
- [x] T27 collapse `/diagram/.gitignore` (R5): pattern rules for the live tree, `!` re-track for Mode A
      ...
      research: rendering
- [x] T28 the index emits BOTH trees on one page with cross-tree links that resolve from a `file://`
...
- [x] T29 **R10, Principle XIV**: `scripts/gate-stamp.py` cannot stamp inside a git worktree - it writes
      ...
      research: rendering
- [x] T30 `migration-plan.md` (FR-021): the new layout, and that a converted map MOVES TREES
      research: rendering
- [x] T31 `dev/pool.md` (FR-022): what the two trees mean, what decides which tree a map goes in, and
      what a session does when a map is converted
      research: rendering
- [x] T32 the remaining ~18 documents carrying literal pool paths - `SKILL.md`, `buildings.md`,
      ...
      research: rendering
- [x] T33 SC-004: content-hash every moved file across the move, ASSERTED by script, not eyeballed.
      Only a `.gen.py`'s path arithmetic and a document's paths may differ
      research: rendering
- [x] T34 SC-003 / FR-008: `git status` shows renames only, and the 18 exhibits are still tracked.
      28 maps before, 28 after
      research: rendering
- [x] T35 SC-007: repo-wide sweep for surviving old two-level paths; every hit is either fixed or
