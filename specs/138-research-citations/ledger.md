# Ledger: every uncited research finding in the diagram skill (FR-001)

*Taken 2026-08-28 by scanning every `## ` heading under `.claude/skills/diagram/research/` (README and SOURCES excluded) for a `**Sources:**` line that says `not recorded` or is missing, then the standalone research documents, the operative and pool docs for inline grounding, the spec research files, and engine comments. One row per finding; the status column is the feature's progress. "Are we done" is this file's open-row count, not an impression.*

**Status vocabulary**: `open` | `re-sourced` (reading agrees; text kept) | `supplemented` (agrees; addition made) | `no-source` (searched, nothing checkable; evidence class corrected, sources line says what was searched) | `contradicted` (record corrected; rule untouched; in the GM report).

## A. Research tree - 117 candidate rows (73 `not recorded`, 44 with no sources line)

*A `no sources line` row is often an entry that cites in its prose (feature 133's entries); the batch normalizes it to a sources line with registered keys, or strikes it as a section header rather than a finding, with the note here.*
...
*Only the historical findings; technical research (packages, CI, tests) is out of scope. A row is closed when its findings are cited in a research-tree entry that the spec file points at, or the spec file itself carries a `**Sources:**` line per finding.*
...
*Read, never edited (FR-006). Each finding is recorded and cited in the tree; the comment is left as it is. Known at the start (a grep for Edo / Tokugawa / Song / jokamachi / county seat in `l7r/**/*.py` hits 9 files):*
...
|---|---|---|---|---|---|
| 3 | `research/cities/capitals.md` "The chancellery meets IN the castle" | Edo's Hyōjōsho sat within Edo castle, so the capital's chancellery meets in the castle (feature 020 drew no chancellery compound on the avenue) | ja.wikipedia 評定所 (READ): "江戸城外の辰ノ口" - outside the castle, across the moat from the Ōte gate | the capital's government avenue / castle program in `settlements/capitals.md`; the Shiro Daika gen | **Resolved by the read (2026-08-28): a median han's administration sat INSIDE the castle enclosure (Kakegawa's 政務所 in the ninomaru goten; 藩庁 "城に"; the jin'ya for castle-less domains) - the GM's in-castle ruling stands as ACCURATE; the Edo Hyōjōsho anchor is struck from the record.** |
| 4 | `research/cities/capitals.md` "Street widths" (and `s.road`'s 26 ft default) | 26 ft is "the Tōkaidō's own width" | ja.wikipedia 東海道 (READ): the 1604 standard was 5 ken ≈ 9 m ≈ 29.5 ft; the 13.8 m Honchō-dōri peg not found (9.9-11.8 m seen) | `s.road` default `lw(26)` on every Mode B map; the 45 ft ōte-suji as "half again the highway" | **GM 2026-08-28: raise the default to ~30 ft on every Mode B map** - an engine change, so its own spec-kit feature (139) after 138 lands; not done inside 138 (FR-006). |
| 2 | `research/archetypes.md` "The 6:4 water-to-dike ratio, and coppiced mulberry" | six parts water to four parts dike; Kuwabata drawn at 76% water per parcel | cssn.cn (READ): "基六塘四" - six dike to four pond; a people.cn page (search summary only) says the reverse; a 7:3 variant exists | the drawn bank width in `build_polder` / `_mulberry_rows`; the Kuwabata dike-pond map; `settlements/archetypes.md` | **GM 2026-08-28: leave as a disclosed regional reading; the map's record updated (kuwabata.notes.md + the entry) so the HTML version carries it.** |
| 1 | `research/buildings.md` "A compound wall is a building, not a boundary line" | neribei / dobei walls are 1.5-3 ft thick at the base; the vocabulary draws them at 3 ft | ja.wikipedia 塀 (城郭): plastered walls "1尺（約300ミリメートル）以上"; 攻城団: frameless neribei ~30 cm, only tsuijibei up to ~1 m | the Mode A wall stroke (3 ft) in `buildings.md`; `pack_audit.structures_on_walls` and the placer clearance are thickness-agnostic; every Mode A sheet in the pool | **GM 2026-08-28: keep 3 ft as a rendering convention for legibility; document the tsuijibei precedent; the HTML map explains it.** Recorded in the entry (class: deviation). |
