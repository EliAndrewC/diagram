...
- [ ] T50 **the lane web under the wider yards** (research: rendering - a lane's SHAPE on the sheet; the physical question of yard size is T49's, already researched and cited) - the web regressions from T49's larger tail. Two engine defects found and FIXED, both latent and both older than T49, which only moved geometry until they showed:
  ...
  - **REMAINING REGRESSIONS, all downstream of the authorized repacking, none yet fixed**: `village_windbreak_is_continuous` 8/32/40 (diagnosed above), `farmhouses_reach_a_way` 15/17, `lanes_bend_like_paths` 21/39/46, `structures_clear_of_trees` 32, `captions_clear_the_ways_they_stand_on` 36, `lanes_reach_something` 47. Twelve in all, against nineteen seed-checks fixed. NOTHING IS PUSHED while these stand (constitution XIII); the GM's call is whether to waive them the way tripwire seeds were waived at the T99 unlock, or to keep fixing family by family
      research: rendering
