...
- [ ] T50 **the lane web under the wider yards** (research: rendering - a lane's SHAPE on the sheet; the physical question of yard size is T49's, already researched and cited) - the web regressions from T49's larger tail. Two engine defects found and FIXED, both latent and both older than T49, which only moved geometry until they showed:
  ...
  - **RE-MEASURED AFTER MERGING MAIN (145/146), 2026-08-29.** The earlier twelve were measured against a baseline main has since moved past, and most were already fixed there. Against main's tip (3c7384da), like-for-like on `make cohort N=24` (which runs `cohort_audit`, NOT the driver's pinned `--batch 24` - `COHORT_BASELINE` is never consulted by that target): main 16/24, this work 14/24, with EIGHT per-seed regressions and EIGHT fixes. The live list: `lanes_bend_like_paths` 10/16/18, `lanes_form_one_network` 13/17, `features_do_not_overlap` 18, `houses_clear_of_lanes` 6, `structures_clear_of_trees` 9.
  - **seed 18's overlap FIXED - the last pass undid the pass before it.** `_thread_the_fabric` routes and clips the connector's inner end clear of every steading (measured: 18.0 ft off the nearest fabric), and then `_pull_back_to_service`, later in the same stage, SNAPS that end onto the lane it joins with no idea what else is standing there - landing it 0.8 ft from a garden's corner, inside the 6 ft the overlap matrix gives every lane. The snap is worth having, so it is now taken only when it does not foul what the threading had cleared. Found by tracing the connector's first vertex through every stage, after two rounds of reading the code produced only plausible wrong answers.
  - **seed 6 diagnosed: the repair's margin is looser than the check's.** The house corner stands 4.09 ft from a 5 ft web lane; `houses_clear_of_lanes` wants `half + 2.0` = 4.5. `_smooth_web`'s string-pull tests its chords with `_clear_touch`, which uses a flat `_TOUCH_GAP` of 4.0 - so the smoother is allowed to draw exactly what the check forbids. Its docstring warns off widening that constant and asks for "a failing seed that names it"; this is one, and the fix is not the flat widening it warns about (7 ft, 6 ft, and 6-for-plots-4-for-houses were each measured and reverted) but the check's OWN width-derived formula, which leaves the common 3 ft lane untouched (3.5 < 4.0) and tightens only the 5-6 ft ways.
  - **seeds 13 and 17 diagnosed, NOT fixed - the whole web hangs off the connector.** Instrumenting `_touch_junctions`'s orphan tail showed the shape plainly: lanes 1-7 are orphans of the connector's component in every one of its twelve passes and nothing ever joins, because the tail joins each orphan TO THE CONNECTOR'S component rather than to the largest one. So when the connector alone fails to touch the web, one detached way is reported as seven detached ways, and the joiner spends its whole ladder trying to drag the web across the map instead of attaching the road. A/B: forcing the snap through (ignoring the fabric entirely) does NOT fix them, so it was never the fabric guard's doing - the connector's end simply never lands on the web here. **FIXED** by making the tail's "network" the LARGEST component rather than the connector's, so the road may itself be the orphan and one 24 ft link does what twelve passes of dragging could not. `lanes_form_one_network` asks whether the pieces are one; it does not care which of them moved to meet the other.
  - **TWO SPECULATIVE FIXES REVERTED AFTER MEASURING**, both recorded at the point of change: a start-outward ladder in `_thread_the_fabric` (that function was never at fault), and the yard/garden footprint-vs-tread test in `_bundle_fits` (whose exclusion note has genuinely expired - the grove got the test in 126 and `features_do_not_overlap` does now forbid lanes x gardens - but which changed no map, so it does not ship unproven)
      research: rendering
