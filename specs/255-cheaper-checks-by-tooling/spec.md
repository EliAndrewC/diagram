...
**FR-004 - independent tool calls go out together.** Every agent contract under `.claude/agents/` gains one
instruction, in the same words: send the reads, greps and fetches you already know you need in ONE message,
...
adopted for all contracts if it holds there; `research.md` reports turns before and after.
...
each adds to a subagent's first-turn input. `research.md` records the breakdown. Anything found that a CHECK
does not need and that the session owns is trimmed (the memory index's line lengths are the session's to
keep short; an agent file's tool list is this repository's); anything that is the GM's or the harness's is
reported with its size, not changed.
