...
- **FR-004 The re-roll loop on stand-in stages.** `generate()`'s loop is exercised with `driver.STAGES`
  ...
  to the second build, the re-roll that helps nothing not kept, the artifacts written - the same assertion
  statements, over stand-in stages, no map (what that no longer proves: FR-006 d). Retry and NoHelp leave the
  roster.
...
- **FR-006 What proves less, stated; the further cuts recorded, not taken.** Every other assertion of the
  sixteen sites is kept on a map that exists or on the stand-in that holds it. FOUR SITES PROVE LESS after
  ...
  (D5); (d) the re-roll loop's assertions are held over stand-in stages, so the reported-verdict assertion
...
- **D1 - the reference's brief is the pool's.** The tests' `Inashiro 4` gains `fixtures_min={'shrine': 1}`
  so the map the gate asserts on IS the shipped map. The GM's ruling - *"I don't care whether the Inashiro
  reference has a different fixture from the brief, so that's fine"* - makes the difference immaterial; the
  alignment itself is this feature's move, because it is what retires the duplicated spec.
