...
- [x] T33 `make done` green (detached); commit; `sync-with-main.sh done`; the closing report with the README correction text, the Mode A rulings question, and any claim left a guess
      research: procedure
      verify: DONE. DONE. make done GREEN detached (71 s, exit 0): 3661 passed, 2 skipped, TOTAL 24211 statements 0 missing 100%, roll census green - the per-generator lock fix holds, where the previous run failed with Kuwabata rolled twice after feature 228 moved its cache key. hooks-test 2 suites green, 19 unchanged. The closing report is specs/229-rule-files-into-research/closing-report.md: the README correction offered not applied (constitution XVII), the Mode A question with the four GM rulings tabulated against what research/buildings.html carries, and what the record still does not know - 113 claims labeled a guess, and the larger number, 108 of the 176 new footnotes being absence notes.
