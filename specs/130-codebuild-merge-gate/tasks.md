...
- [x] T028 (DECIDED WITHOUT THE 2xlarge RUN, on evidence: e2511c85/d73cac86 show the test phase at 241/240 s on 36 vCPU against 221-225 s on the 22-thread laptop - the suite is not CPU-bound at that width, so 72 vCPU cannot help reference scope; compute stays BUILD_GENERAL1_XLARGE, RATE_PER_MIN unchanged. Recorded in timings.md; reopen when FULL/cohort numbers say otherwise) [US3] 💵 (~$1.20) T-measure: the same `make done` on `2xlarge`; record both numbers and provisioning with the custom image in `timings.md`; choose the compute type by the number and set it on both projects; update the rate constant AND the Lambda's `RATE_PER_MIN` if it changes (spec baseline table says where)
