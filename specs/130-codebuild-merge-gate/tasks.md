...
- [x] T019 (2026-08-25 build b2056da1 SUCCEEDED on the check project (privileged, MEDIUM): Dockerfile.ci built and pushed to ECR as :latest, marker image/latest.txt written; two earlier attempts each found one defect (system pip on 26.04, Docker Hub's 429 - now a uv venv and an ECR Public base). Prompt answered by the session under the GM's authorization of the same day, recorded in the logged reason) [US3] 💵 (~$1, one image build) `make ci-image`: prompted, cancel-by-default, reason logged via the existing `LOGBYPASS`, refused non-interactive; builds on the check project and pushes to ECR; update both projects' `image` to the ECR URI
...
- [x] T021 (the prompt proceeded with a reason and logged `permitted` (dev/bypass-log, target ci-image), three times - the entries say the session answered and quote the GM's words) [US3] **STAYS QUIET**: `make ci-image` in a terminal with a reason proceeds and logs `permitted`
...
- [x] T023 (applied 2026-08-25 as `tempadmin` from buildspec/verified-deny-policy.json; verified with the session key: `put_object verified/probe.json` -> AccessDenied, `go/` still writable. The tempadmin key was pasted into the session transcript to do this - rotate or delete it) [US3] **Admin-key task (GM)**: bucket policy denying `s3:PutObject` under `verified/` to every principal except `gm-assistant-codebuild-role` (research R8). Ticked only when a `put_object` with the session's key returns `AccessDenied`
...
- [x] T035 (the first real merge through `sync-with-main.sh done` IS this push, 2026-08-25 - reference scope on gm-assistant-merge; the FULL variant (T063) stays open because the FULL sweep is red on the pre-existing ratchet seed 41 (`fields_clear_of_road`), a map defect outside both features' scope) [US1] 💵 (~$0.45, or $0 if T024's tree is unchanged) First real merge through `sync-with-main.sh done` on THIS feature's own work: build lands on GitHub `main`, clone fast-forwards, `/diagram` fast-forwards, render-sync runs in the mirror, GitHub and the mirror show the same commit
...
- [x] T080 (RAN 2026-08-25, build a6e2afe6: prompt answered under the GM's authorization, `permitted` entry committed, build parked and released, the door opened on the committed entry, every pool map + ratchet + floors ran (3,599 passed) and perf-gate took BOTH bookends in-build (two `codebuild` snapshots returned as artifacts into dev/perf-log/). The build was RED on two counts, neither a defect of this feature: cohort seed 41 fails `fields_clear_of_road` (the standing map residue, out of scope by the spec's own assumption - no failing seed is addressed), and one of this feature's tests read SPECIFY_FEATURE from the environment the build exports (fixed). 18 billed min, $1.44) [US8] `ci-check FULL=1` end to end on the check project 💵 (~$1): prompt, committed entry, parked build, local reference, release, FULL suite with both bookends, FULL verified record (restores the check-first proof for FULL that the first amendment's round 2 removed)
