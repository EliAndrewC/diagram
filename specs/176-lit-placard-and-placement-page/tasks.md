...
- [ ] T01 FR-001/FR-002: `page.css` - the placard's name keeps `--ink` while its class is lit, a
...
- [ ] T02 FR-005: `tests/tools/test_placement_stages.py` - every stage in `STAGES` has a `NOTES`
...
- [ ] T03 FR-003/FR-004: the three notes (`stage_waterward`, `stage_pond_stock`, `stage_labels`) and
...
- [ ] T04 D3: the `stage_waterward` docstring no longer says it is called from the hinterland
      research: rendering
      verify: the docstring matches `STAGES`

- [ ] T05 FR-006: `make placement-stages` re-plates the page; the committed HTML has no
