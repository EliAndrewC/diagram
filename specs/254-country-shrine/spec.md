...
- Amendment during implementation (2026-09-19): four Decisions Recorded rows changed on the verification agents' findings - the bell knob's class to guess, the residence row's disclosure of uncounted village-scale prevalence, the precinct's anchor a modern rebuild, the one-roof example an urban temple. The amendment resets the counter; round 3 (MODE 3) is owed on these rows.
