...
- [ ] T02 the census plugin, its verdict and the roster (FR-003, FR-004); the Makefile loads it; each
      ...
      research: rendering
- [ ] T03 one roll per spec: the unified child roll, `Report.manifest`, `hamlet()`/`report()` read it (FR-001)
      research: rendering
- [ ] T04 the first-wave lock and the roll cap (FR-002, FR-010's cap); proved with two producers
      research: rendering
- [ ] T05 the child roll-out: the five closures lifted and run by name, the immune and CLI tests, the
      fan-out's serial half, the regen child (FR-007); child equality per closure
      research: rendering
- [ ] T06 tests do not render: the default, the opt-outs, the census's render refusal (FR-006)
      research: rendering
- [ ] T07 first-attempt seeds (FR-005); collection order (FR-010)
      research: rendering
