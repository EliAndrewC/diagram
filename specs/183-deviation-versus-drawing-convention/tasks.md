...
- [x] T07 FR-009/FR-010: regenerate the reference hamlet's page and read the bund beans' and the well's modals; `make done` green; the answer to the GM
      research: rendering
      verify: DONE. Reference page regenerated; the bund beans, well, shrine and stream modals open "Note: we have rendered/drawn ..." and the beans gain a fourth question. First `make done`: 2,961 passed but the 100% floor RED at 22,621 / 60 uncovered - all 60 in `ci/dispatch.py`, which main merged INTO THE CLONE MID-RUN (the clone-sync hook, 16:56; the run had collected its tests at ~16:54), so the new file was measured and its new tests never ran. Re-run on the stable tree: GREEN in 288 s, 2,976 passed, 22,621 statements 0 uncovered 100%, hamlet floor 12,449 / 0 / 100%; `hooks-test` re-stamped after the merge changed scripts. `PAIR_OK` given: no drawn ink moved. Finding for the GM: a merge landing mid-gate is not detected - the run record keys the tree at the END, so a merge that lands after collection is vouched for by tests that never saw it; here the floor caught it, but a merge with its own tests present would have passed silently