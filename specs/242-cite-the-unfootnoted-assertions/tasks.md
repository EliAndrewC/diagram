...
**The work list is DERIVED, and this file is written from it** (FR-001). The per-page counts below come
from `measure/inventory_census.py --per-page`, which reproduces all four reader reports' own stated
totals before it will report anything. They are the BARE items on each page - the ones carrying no
inline marker. Twenty-two of them across the whole record are closed by feature 238's R7 (the fourteen
roster-hidden claims, the six sections that disclosed with no footnote at all, the one
`settlements.html` item and the caravan inn), and R7 does not say which page each belongs to, so **each
count below is an upper bound**. `settlements.html` and `presentation.html` are absent because 238
closed them outright.

**Within a page, work strongest first** - HIGH, MEDIUM-HIGH, MEDIUM, LOW-MEDIUM, LOW (FR-012). The page
is the unit of work and of checking because `record-format` and `quote-check` read a page; the
confidence order runs inside it.
