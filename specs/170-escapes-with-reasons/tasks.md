...
- [ ] T02 FR-001: a reason floor in `_hookmatch.py` - `escape_reason(cmd, token)` returns the text
      ...
      research: procedure
      verify: a table of vectors through the CLI - a bare token, `GATE_OK: ok`, `CI is down`, a
      quoted reason, a reason with punctuation - each asserted in BOTH directions

- [ ] T03 FR-001: every command guard refuses a bare token and says the compliant form, using the
...
- [ ] T04 FR-002: the two silent permits record - `make-only` (via a distinct verdict from
...
- [ ] T05 FR-003 / FR-003b: the content, environment and make-variable escapes take the same floor,
...
- [ ] T06 FR-003: `not-an-escape` (`SWEEP_OK`, `REMOTE_OK`) owes nothing, and the derived census
...
- [ ] T07 FR-004: a finished, unsurfaced background run is reported at the next prompt and at turn
...
- [ ] T08 FR-005: `main-tree-hooks.sh` reads the session's `cwd` from the hook payload, so a write
