...
- [ ] T01 FR-001/FR-004 (runs AFTER T04 - the exemplar is authored in HTML): the footnote form and the mechanical test (`tests/interactive/test_footnotes.py`); a worked section footnoted by hand as the exemplar (the farmstead fixtures entry: the coop's sources, one footnote each)
      research: rendering
