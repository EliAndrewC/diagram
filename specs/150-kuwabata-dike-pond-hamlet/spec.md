...
| Reed marsh keeps OFF every earthen mound - a perimeter dike band or a pond's mulberry bank - and the waterward strip's inner edge follows the dike's outer FACE (T54) | accurate | the dike is dredged pond-mud heaped, packed and PLANTED (a bare bank gullies and slumps, so planting it was the standard remedy - `research/archetypes.md`), and the un-reclaimed fluctuating wild lies OUTSIDE it (`settlements/archetypes.md` "Polder waterward fringe"); reeds root in shallow standing water, so wet ground abuts a mound and never crosses it. GM 2026-08-28: "the hazy blue that denotes the marsh is clearly overlaid on top of the greenery of the earthen mounds" | `settlement/land/wet.py` `marsh` (the keep-out, every role), `hamletgen/water.py` `dike_face` + `stage_waterward`, `settlements/water.md` |
| A parcel stops at the ditch that bounds it: a channel crossing a parcel CUTS it, and the outline is projected onto the band's edge rather than shoved off it (T55) | accurate | the channels are laid on the ideal lattice line while the parcels wander off it, so a holding ended up across its own ditch and its crop rows were drawn under the water; cropped ground stops at the ditch's far shoulder. GM 2026-08-29: "one of the vegetable grounds overlaps with the irrigated channels which run between the vegetable grounds and the ponds" | `waterfields/polder.py` `_plots_clear_of_channels` (with both declined approaches recorded there), `hamletgen/water.py` `fit_polder` |
