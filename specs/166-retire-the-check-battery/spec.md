...
- **FR-011**: The documentation sweep MUST remove every place the docs describe or instruct the Mode B
  ...
  magistracy diagrams, There still are automated checks and those survey [serve a] valuable purpose because
  those maps are generated by hand rather than by a scripted process."* (Quoted exactly, with the
  speech-to-text artifact bracketed rather than silently corrected - an earlier draft of this requirement
  normalized it to "those serve a valuable purpose", and a requirement must not rest on words attributed to
  the GM that the GM did not say.)

  **The sweep covers the skill's OPERATIVE docs** - `SKILL.md`, `buildings.md`, `settlements.md` and the
  files they index, `dev/`, and the skill's `CLAUDE.md`. It MUST NOT edit historical `specs/NNN-*/`
  artifacts, this feature's own `gm-request.md` (which constitution Principle V forbids editing), or the
  migration record FR-009 requires. 30+ past feature artifacts describe the battery as part of the scripted
  process; they are the RECORD of what was true then, and rewriting them would be falsifying history to
  match the present.
...
- **SC-007**: The skill's OPERATIVE docs contain no live description or instruction of the Mode B
  post-placement battery, **including mentions that do not name `check_village`** - `SKILL.md`'s frontmatter
  (*"parametric generator plus an automated validator gate"*) and `SKILL.md:78` (*"an automated validator
  gate"*) are the measured instances - and each removed mention is recorded as Mode B in the sweep record.
  SC-001 is name-based and by construction misses exactly these.
- **SC-008**: Mode A's check doctrine and its apparatus are INTACT after the sweep - `pack_audit`,
...
### Round 4 - delta review of the Mode A carve-out, 2026-08-30: CHANGES REQUIRED, all three applied

Scoped to FR-011, SC-007 and the `gm-request.md` entry, the rest of the spec being FAITHFUL at round 3. The
reviewer confirmed FR-011 does real work FR-006 cannot - FR-006 is NAME-based, and the live docs describe
the battery **without naming it** (`SKILL.md`'s frontmatter and `SKILL.md:78`, *"an automated validator
gate"*) - and confirmed the carve-out is drawn at the right boundary by testing the obvious counter-case:

> I tested the obvious counter-case: `legacy-hand-authored-pool/` holds 18 hand-authored **Mode B**
> exhibits... On the GM's stated reason - *"generated by hand rather than by a scripted process"* - those
> look like they should be carved out too. They should not: they are *"never regenerated, never re-gated"*.
> No check runs on them, so there is no live doctrine to preserve... **Mode A is in fact the only
> hand-generated class with live checks.**

| # | finding | change made |
|---|---|---|
| 1 | **The spec MISQUOTED the GM.** FR-011 rendered the carve-out as *"those serve a valuable purpose"*; the GM said *"those survey valuable purpose"*. The session silently normalized a speech-to-text artifact into tidier English. Every other GM quotation in the spec is verbatim, and this repository's tooling deliberately refuses to auto-correct GM request text. | Quoted exactly, with the artifact BRACKETED (`those survey [serve a] valuable purpose`) and the error recorded in the requirement itself. A requirement must not rest on words attributed to the GM that the GM did not say. |
| 2 | **FR-011 did not say what "the docs" are, and the literal reading swept the historical record** - 30+ `specs/NNN-*/` artifacts describe the battery as part of the scripted process, as does this feature's own `gm-request.md`, which Principle V forbids editing. | FR-011 now names the OPERATIVE docs and forbids editing past feature artifacts, the GM request, and FR-009's migration record. Those are the record of what was true then; rewriting them would be falsifying history to match the present. |
| 3 | **The sweep half had no success criterion - only the exception did.** SC-001 is name-based and by construction misses exactly the mentions FR-011 was added to catch. | New SC-007 measures the sweep itself and names the two measured un-named instances; the Mode A criterion became SC-008; the ordering aside is fixed. |

**Finding 1 is the one to carry.** The habit feature 163 named was propagating a correction incompletely.
This is a different and worse one: **improving the GM's words while quoting them.** It reads as courtesy and
it is falsification - the quotation is the authority a requirement rests on, and a tidied quotation is an
authority the GM never granted. The rule is simple and absolute: a verbatim block is transcribed, never
edited; an artifact is bracketed, never smoothed.
