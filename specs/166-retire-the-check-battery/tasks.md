...
- [X] **T01** Baseline on UNMODIFIED code: `git worktree add --detach /tmp/base166 HEAD`, `make done` there,
...
- [x] **T03** Replace the ladder's accept criterion (`len(f2) <= len(failures)` over the gate's whole
      ...
      research: procedure
- [X] **T04** `hamletgen` imports nothing from `check_village`. Prove it: a test that the module graph is
...
- [X] **T07** Batch `water_frame` + `field` + `sink` - the water and field engine's checks.
      verify: `make quick`
      research: procedure
- [X] **T08** Batch `seat` + `homesteads` - the cluster and farmhouse placers.
      verify: `make quick`
      research: procedure
- [X] **T09** Batch `track` + `web` - the way network.
      verify: `make quick`
      research: procedure
- [X] **T10** Batch `appurtenances` + `notice` - yards, gardens, wells, the board.
      verify: `make quick`
      research: procedure
- [X] **T11** Batch `hinterland` + `woodland` + `windbreak` + `bamboo` - ground cover and the belts.
      verify: `make quick`
      research: procedure
- [X] **T12** Batch `crossings` + `frame` - bridges, and the frame the crop holds open.
      verify: `make quick`
      research: procedure
- [X] **T13** Batch `finish` - the terminal stage, which feature 163 measured as the last-changer for 75%
      ...
      verify: `make quick`
      research: procedure
- [X] **T14** The 17 completeness ratchets (they read no manifest key) become STATIC tests over the code -
      a new feature type added without a registry entry must still turn something red.
      verify: `make quick`
      research: procedure
- [X] **T15** The whole-map properties become ONE seed-sweep test over a documented seed set, in
      `tests/full/`. Say in its docstring which properties it carries and why a seed sweep is the right
      home for them.
      research: procedure
- [X] **T16** The URBAN checks (39 segments carrying an `if URBAN:` branch): no placer exists to migrate
...
- [X] **T17** Delete `check_village/`, `tests/check_village/`, `tests/gate/`, the 105 frozen fixtures, the
...
- [X] **T18** Per-rule confirmation that every research finding whose only operative statement was a check
...
- [X] **T19** Rewrite `dev/gate.md` to carry the successor doctrine in the GM's own words (FR-010): when
      ...
      research: procedure
- [X] **T20** The doc sweep, per MENTION (FR-011): Mode B battery doctrine goes; **Mode A's stays**.
      ...
      research: procedure
- [X] **T21** Record where each of the 147 rules now lives (FR-009), so "what happened to rule X" is
