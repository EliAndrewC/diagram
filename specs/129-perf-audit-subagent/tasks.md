...
- [ ] T005 💵 (~$1.20) CodeBuild noise floor: three `ci-check TARGET="perf LABEL=129-noise-{a,b,c}"` on one commit; spread in research.md + timings.md; report to the GM if materially different from 0.7%/1.7%
...
- [ ] T013 `perf-profile SEED= STAGE=`: cProfile of one stage, derived top-25 table tracked (kilobytes), raw `.prof` under gitignored `dev/perf-raw/`; archive step degrades when no repository is configured (FR-011b); **FIRES/QUIET**: the table exists and is under 8 KB; the `.prof` is not tracked
...
- [ ] T020 This feature's own bookends: `-end` taken, bands evaluated, records produced through the real subagent (the first use), `make perf-review` green

## Closing

- [ ] T021 `make done` green; hooks-test green; `sync-with-main.sh done` through the gated route
- [ ] T022 Report to the GM: the identity finding, the cProfile number, the CodeBuild floor, the archive repository they create
