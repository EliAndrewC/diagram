...
- [x] T013 (l7r/diagram/tools/perf_profile.py; proven 2026-08-25 on seed 4 stage `seat`: the derived table dev/perf-log/20260825T123324Z-profile-129-seed4-seat.txt is 3,951 bytes and tracked, the raw .prof sits under the gitignored dev/perf-raw/ (git check-ignore confirms), and the archive step reports itself skipped without PERF_ARCHIVE. LATER THE SAME DAY the GM created https://github.com/EliAndrewC/mapgen-perflogs - it is now the default archive, pushed with the PAT, and the seed-4 profile is its first artifact) `perf-profile SEED= STAGE=`: cProfile of one stage, derived top-25 table tracked (kilobytes), raw `.prof` under gitignored `dev/perf-raw/`; archive step degrades when no repository is configured (FR-011b); **FIRES/QUIET**: the table exists and is under 8 KB; the `.prof` is not tracked
