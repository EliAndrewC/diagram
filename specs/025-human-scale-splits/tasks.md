...
- [x] T502 Stop-work procedure: final commit, then `scripts/sync-with-main.sh done` from the clone (locked pull+push + render-sync). NOTE: the 19 frozen legacy maps' renders are now write-once git exhibits (main commit 0719da0) whose gens never run - they keep their old stamps; only scripted-tier maps re-stamp under the new engine fingerprint, and their drawn bytes are proven unchanged by the oracle
