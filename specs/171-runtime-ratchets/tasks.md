...
- [ ] T01 baseline: `make hooks-test` and `make done` green before any change
      research: procedure
      verify: both green, so anything red afterwards is this feature's

- [ ] T02 FR-003 / FR-005: the ratchet TABLE - one row per target, carrying the pinned baseline, the
...
- [ ] T03 FR-004: the derivation, `min(HARD_CEILING, max(baseline + 4, int(baseline * 1.3)))`, with a
...
- [ ] T04 FR-001: `make quick` FAILS at 15 s or more, measured on THE RUN ITSELF
      research: procedure
      verify: driven at 14 s (passes) and 15 s (fails) with an injected duration, not by waiting

- [ ] T05 FR-002: `make done` fails at its ceiling, with ONE RULE PER REGIME - while the pinned
...
- [ ] T06 FR-009: only runs that DID THE WORK are compared, and like with like - a short-circuited
...
- [ ] T07 FR-008: a failure SAYS WHAT TO DO, in the house style, naming the target, the number, the
      ceiling and the baseline it came from
      research: procedure
      verify: the message asserted, as `QUICK_BUDGET`'s already is

- [ ] T08 FR-010: moving a pinned baseline in EITHER direction leaves a written reason at the point of
      change
      research: procedure
      verify: a test that fails if a baseline row carries no reason

- [ ] T09 FR-007 is OPTIONAL and is DECLINED for now, with the reason recorded: `quick` writes no
...
- [ ] T10 D1 and D2 are the spec author's decisions, not the GM's - carry them forward unchanged and
