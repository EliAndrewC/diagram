...
**`ALL CHECKS PASSED`** on `pool/provincial-cities/minami.json`, at the SPECIFIED population of
**2,600** (FR-010). `check_village.py pool/provincial-cities/minami.json --capacity` reads
**SIZED_AND_PACKED** and the map draws **exactly 520** dwellings against its 520 target (the 7%
population band was removed on 2026-07-27; the figure is now met on the nose). The generator is
deterministic (identical source -> byte-identical manifest, verified) and now lives under
`pool/provincial-cities/`, so `test_villages.py` gates it with every other map.

The wall was **sized to the population**, not the population trimmed to the layout. An intermediate
pass did the latter - dropped the lay figure to 2,000 and pinned the old ring - and that was
reverted: if the enclosure cannot hold the declared figure, the enclosure is what is wrong. The ring
came back from `plan_city` (FR-011) at a circular 462x462 (was 431x400), paid for by two measured budget
corrections. Full reasoning, the registry map, and the five expensive lessons are in
[`pool/provincial-cities/minami.notes.md`](../../.claude/skills/diagram/pool/provincial-cities/minami.notes.md).

Remaining optional polish, none of it blocking: run the `settlement-review` subagent (the
independent Mode B pass for the residue a green gate cannot see - glyph legibility, feature FORM,
whether open ground is a real feature), and land the research/doc tasks T14-T17 below.
...
- [x] T12 [US1] Iterate to zero failures on the single-map loop. Use `s.open_seat(...)` to ask the engine where a feature fits rather than guessing coordinates, and `site_justice.py` for any feature governed by interacting rules. **Confirm `city_capacity` reads `sized_and_packed` on the FIRST derivation** - if it wants a resize, fix the budget model, not the wall (SC-003). **DONE, and SC-003 was exercised in earnest**: the first derivation read `densify`, then `enlarge` once the population was restored, and the fix went into the budget model (`temple_precinct_px2`, the laneway-excess line) rather than into a hand-picked wall. Final verdict `sized_and_packed`.
- [x] T13 [US4] Verify the untested branches actually RAN, per the "a check that never runs looks like a check that passes" diagnostic: `python3 check_village.py pool/provincial-cities/minami.json | grep -c city_wall_tower_coverage` must be non-zero, and the manifest's budget must contain the `extras` line. A zero here is the bug.
...
- [x] T18 Write `pool/provincial-cities/minami.notes.md`. **DONE** - carries the design decisions, the wall-sizing reasoning, the placer/registry map, and the five lessons that cost the most to learn.
...
- [ ] T23 Stop-work procedure: commit in the clone, then `scripts/sync-with-main.sh done` from inside it (locked pull + push, then render-sync). Never force-push.
