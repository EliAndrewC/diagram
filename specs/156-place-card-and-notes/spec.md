...
**Feature Branch**: none - this project stays on `main` (`SPECIFY_FEATURE=156-place-card-and-notes`)

**Created**: 2026-08-29 as feature 154; RENUMBERED to 156 on 2026-08-29 when the sync surfaced a
peer session's own `154-kosatsuba-placement-knob` already on main, and a `155-mains-red-floor` behind
it. The concurrent-sessions protocol renumbers the claim that has not landed, which was this one -
its number-claim push had been refused by the review gate for want of a fidelity verdict, so nothing
of it was on main to collide with.
...
| A count of drawn dwellings is EXACT and carries no tilde; only the population is tilde-marked | measured | The GM, 2026-08-29: "that actually IS an exact map feature - the number of farmhouses listed should be whatever is actually displayed on the map itself." The count is a statement about the sheet the reader is looking at; the population is an inference from it at five to a household | `interactive/place.py` `size_sentence`, and its test |
| A hamlet and a village count FARMHOUSES; a town and a city count NON-FARM DWELLINGS and no farmhouses at all | measured, on a convention the GM set | The GM, 2026-08-29: the upper tiers' non-farm dwellings are "enumerated and known and ... actually exact and rendered", while both are "surrounded by significant farm fields and farmer populations, which are deliberately not all rendered on the map. Therefore, that number should not be included." Implemented by excluding houses inside a drawn `agricultural_district` quarter - the only farm/non-farm distinction the manifest carries, recorded as a limit in `dwellings_shown` | `interactive/place.py` `Kind.excludes_farms`, `dwellings_shown` |
| A town's population INCLUDES the surrounding farmers; a provincial city's does NOT, and each card says which | accurate (setting canon; Imperial census convention) | The GM, 2026-08-29: a town's figure "includes the nearby farmers who are counted as being part of the town, Not all of which are depicted on the map"; a city's "includes samurai country estates, only some of which are depicted on the map, but does not include farmers ... each of the farms being shown belong to village districts and counties. which are administered and counted separately in the Imperial census." A population figure whose scope is unstated is a figure a reader will misread | `interactive/place.py` `Kind.population_note`, `settlements.md` |
