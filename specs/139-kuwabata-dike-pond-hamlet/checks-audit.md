# The checks audit for feature 139, under the GM's rule (2026-08-29)
...
## What feature 139 actually added: one check, and it is 147's
...
| count the title placard as frame-setting | `crop_hugs_content` -> `map_frame_hugs_its_content` | KEPT (renamed) | re-apply: it prevents a FALSE POSITIVE against 147's title-pocket behavior |
...
- **`polder_parcels_are_organic`** - the wander is one knob in `_polder_parcels`. Systemic? No; a unit
  test on the vertex and square-corner counts is the honest guard, which is what feature 147's
...
**`waterward_strips_run_off_the_frame`** (feature 147). The failure it catches needs no placer to
