...
- [x] T004 **GM**: confirm no other session holds unpushed diagram work; create the GitHub repository (name per Assumptions, or say otherwise); generate a fine-grained PAT scoped to it (Contents rw); create the `main` ruleset (block force pushes, restrict deletions)

## Phase 2: Prepare the pieces (all in the clone, nothing pushed)

- [x] T005 Write the new repository's CLAUDE.md: one project, the diagram loop, the command map, the session-clone rules with the new root, numbering continues from 132, the constitution as it stands - by editing the current one down, not from scratch
- [x] T006 Write gm-assistant's post-split CLAUDE.md: diagram loop removed, `/diagram` listed as moved with the URL, the retained hooks table, the spec-number claim protocol unchanged
...
- [x] T008 `webapp/Makefile` gains the `hooks-test` phase (copied from the diagram Makefile) so gm-assistant's retained guards are RUN (FR-002); the four repository-wide guards (`gate-stamp.py`, `review-gate.sh`, `gate-hooks.sh`, `guard-file-hooks.sh`) trimmed to each repository's areas per FR-001's table, companions updated; `.claude/agents/` split per the table. `.claude/settings.json` for the new repository: check in rehearsal whether `$CLAUDE_PROJECT_DIR` resolves in hook commands; if yes use it, else the new absolute root. gm-assistant's `settings.json`: drop ONLY the diagram-only hooks (make-only, guard-file, gate); every COPY-TO-BOTH item in FR-001's table stays in gm-assistant untouched (`.specify/`, `docs/`, `container-scripts/`, `ruff.toml`, the content hooks)
- [x] T009 `docs/session-clones.md`, `docs/container.md`, `scripts/launch-container.sh`, `container-scripts/append-system-prompt.md`: the second mount, the new root, the pointer in each direction
- [x] T010 Feature 130's spec, plan and tasks: the route/delta section shrinks to "engine code vs docs" (the whole repository is the diagram now); record the change in its Review history as a consequence of 131, not a new request
- [x] T011 Features 129 and 130: `**Blocked by**: 131-split-diagram-repo` in the spec header and a `T000` task "verify 131 landed: this directory is in the new repository and `make done` is green there" at the top of `tasks.md`

## Phase 3: Rehearsal (FR-011) - the whole procedure on a throwaway

- [x] T012 Fresh clone to `/tmp/split-rehearsal`; `git filter-repo` with the path list from R1/T002; record the resulting commit count and repository size
- [x] T013 In the rehearsal repository: apply T005/T007/T008/T009; `container-scripts/setup-dev-env.sh --check`; `make done` and `make hooks-test` in a clone under its `.clones/` - counts equal T001 (SC-001)
- [x] T014 `git log --follow` on ten engine files chosen by `shuf` - full history present (SC-002); record the ten
- [x] T015 Feature-127 quickstart in the rehearsal repository: every refusal reproduces (SC-005)
- [x] T016 Rehearse the gm-assistant side on a second throwaway clone: the removal commit, the retained scripts, `webapp/make done` green, the retained hooks fire and the diagram-only ones are absent (SC-003)
- [x] T017 Reference sweep grep in both rehearsal trees: zero LIVE hits (SC-004); record the count of HISTORICAL hits left alone
- [x] T018 Discard both rehearsal trees; record under Rehearsal below that every check passed, with the numbers (SC-006)
...
Both sides rehearsed on throwaway clones from the committed prep state (`3fdf3057`):

| check | diagram side | gm-assistant side |
|---|---|---|
| extraction | 1,327 commits, 224 MB `.git`, `filter-repo` 2.6 s (`--no-local` clone required - a hardlinked clone is not "fresh") | 1,378 commits after the removal commit |
| `make done` | **3,465 passed / 2 failed** (the two known gitignored-artifact failures, identical to the baseline; +1 = the new `render_cache` test), test phase 165 s | `webapp/make done` **green**, 553 passed |
| `hooks-test` | 12 suites | 11 suites (`make-only` moved) |
| history | `git log --follow` on 10 random engine files identical to gm-assistant | - |
| `setup-dev-env.sh --check` | OK | - |
| feature-127 refusal | the harness hook refused a bare `python3 -m l7r.diagram.tools.mapcheck` in the rehearsal tree | - |
| sweep | zero LIVE `/gm-assistant` instructions; the rest are dated spec artifacts, test fixtures, prose | same |

Found and fixed on the way (Principle XIV): `scripts/test_hooks_cases.py` carried literal em/en
dashes that the webapp's repo-wide dash scan flagged - a pre-existing failure in main, now escapes.

## Real run (T019-T025)

- The GitHub repository `EliAndrewC/diagram` had the GM's "Initial commit" (the LICENSE); the
  extracted history was merged with it (`--allow-unrelated-histories`) rather than overwriting it.
  The merge commit's message says "README" where it should say "LICENSE" - left as history.
- Repository name is `diagram`, so the container path is `/diagram` (the spec assumed
  `l7r-diagram`); `CLAUDE.md`'s `container-workdir` marker says `/diagram` and mounts gm-assistant
  read-only at `/gm-assistant`.
- CodeBuild `GITHUB_REPO` repointed on both projects with the session's own key (T021). The PAT
  already covers the new repository (GM, 2026-08-25). **The `main` ruleset on the new repository is
