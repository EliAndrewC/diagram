...
- **Docs-only diffs skip the gate - and since feature 132 the gate skips itself.** `make done` answers `already verified` in seconds when its engine content (every `.py` under the skill outside `tests/`, plus pool gens/manifests) is unchanged since the last green run - the same two keys the remote gate uses to skip a build; docs, tests, the Makefile, config and `scripts/` do not count, exactly as they do not owe a build (GM 2026-08-25, FR-024 for tests). So running it after a docs commit is now free; not running it is still fine.
