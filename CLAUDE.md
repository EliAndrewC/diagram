...
- **Tweaks and iteration** (wording fixes, regenerating one item, fixing one bug) → just do the work directly. The constitution still applies, but the formal spec/plan/tasks flow is overkill. **EXCEPTION - diagram ENGINE code always has a spec-kit feature** (GM 2026-08-24, feature 130 FR-011: *"Anything involving the diagram skill is sufficiently complicated to require a spec kid feature"*): a push whose delta touches `l7r/**/*.py`, `tests/`, or `pool/*.gen.py|*.json` - the code the tests exercise; not the Makefile, pyproject, the lockfiles or the hooks, which shape how the gate runs and are covered locally - takes the GATED route, and the gated route refuses to dispatch unless a named feature is complete (no open task, spec FAITHFUL). A docs-only change under the skill, or anything outside it, needs none.
...
- **Docs-only diffs skip the gate - and since feature 132 the gate skips itself.** `make done` answers `already verified` in seconds when its engine content (every `.py` under the skill, tests, pool gens/manifests) is unchanged since the last green run - the same two keys the remote gate uses to skip a build; docs, the Makefile, config and `scripts/` do not count, exactly as they do not owe a build (GM 2026-08-25). So running it after a docs commit is now free; not running it is still fine.
