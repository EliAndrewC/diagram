...
- **Tweaks and iteration** (wording fixes, regenerating one item, fixing one bug) → just do the work directly. The constitution still applies, but the formal spec/plan/tasks flow is overkill. **EXCEPTION - diagram ENGINE code always has a spec-kit feature** (GM 2026-08-24, feature 130 FR-011: *"Anything involving the diagram skill is sufficiently complicated to require a spec kid feature"*): a push whose delta touches `l7r/`, `tests/`, `pool/*.gen.py|*.json`, the Makefile or `pyproject.toml` takes the GATED route, and the gated route refuses to dispatch unless a named feature is complete (no open task, spec FAITHFUL). A docs-only change under the skill, or anything outside it, needs none.
...
- **Stop-work ritual, EVERY time you stop** (task done, milestone, or pausing for GM input): commit in the clone, then run [`scripts/sync-with-main.sh`](scripts/sync-with-main.sh) `done` from inside it. **TWO ROUTES TO MAIN, chosen by the delta, never by you**: no diagram engine code in YOUR commits -> DIRECT (a locked fast-forward push to GitHub main, free, today's guards); engine code -> GATED (`make ci-merge`: five conditions checked locally, then a CodeBuild build merges the LATEST main into the work, runs `make done`, and fast-forward-pushes the result to GitHub main; the clone and the mirror fast-forward to it). `done FULL=1` runs the full sweep on CodeBuild after the local prompt. There is NO local override of the gated route - a delta that cannot be dispatched stays in the clone. **Never `git push --force`** - it is the one thing that overwrites other sessions' work.
...
- **Paid, prompted targets are the GM's to answer at a terminal**: `make ci-image` (the build image) and any `FULL=1` dispatch. A session never answers its own prompt - not through a pseudo-terminal, not through an environment variable (`spec-fidelity`, 2026-08-25: EXCEPTION). `make ci-check` (reference scope) and the gated push need no prompt.
