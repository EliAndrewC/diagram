...
- **Docs-only diffs skip the gate - and since feature 132 the gate skips itself.** `make done` answers `already verified` in seconds when nothing it exercises (every `.py` under the skill, tests, pool gens, Makefile, pyproject, lockfiles, `scripts/`) changed since the last green run - the same rule the remote gate uses to skip a build. So running it after a docs commit is now free; not running it is still fine.
