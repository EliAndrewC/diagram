...
| `make _reference` (internal) | one seed of the reference hamlet, and nothing else | **29 s**, or ~0 on a roll-cache hit |
...
The `PreToolUse` guards, most of them about efficiency. **The rule is
...
| **`make agent-census`** (feature 251, GM 2026-09-19) | nobody knew which subagent check the tokens went to - the tiering question was being argued from intuition | reads every subagent transcript: runs, turns, input, output, thinking and the model each run ACTUALLY used, per agent type; usage folded per message id at its maximum (a transcript repeats a message per content block). First finding: input is 75-90% of a check's cost, and the largest consumer is the ad-hoc `general-purpose` agent - 72 runs dispatched with no model, 62 of them on Fable |
| **`make quote-verbatim` / `record-prepass` / `size-table`** (feature 251) | the mechanical first step of three checks done by a model turn by turn - and done WORSE: a model reads tokens, not characters | three scripts run BEFORE `quote-check`, `record-format` and `size-audit`, their output named in the agent's prompt. On its first slice the verbatim script found two character differences a recorded Opus run had missed and showed one of that run's DIFFERS to be false (the live page carries the quoted characters) |
| **the batching line in seven agent contracts; `make source-pages`** (feature 255, GM 2026-09-19) | a check's cost is its turns times its context (251 R9), and six tooling candidates claimed to cut one or the other | each candidate re-run on RECORDED cases and kept only if it hit every recorded finding for less. ONE stood: "send the reads, greps and fetches you already know you need in ONE message" - a later review round in a third fewer turns and a quarter less weight, nothing missed. Every candidate that cut what a check READS (registry entries handed to `source-applicability`, scoped text for `record-format` and `quote-check`, `review-facts` and a shorter contract for `settlement-review`) LOST recorded findings and was reverted: what a judging agent reads on its way to a finding is part of how it finds it. `make source-pages` saves a page's full text where a fetch gives an extract - with it `source-reader` missed nothing and corrected a recorded false CONTRADICTED, at a quarter more weight, so it was put to the GM, who adopted it (2026-09-19): the session runs it BEFORE `source-reader` and the agent greps the saved pages. The fixed context measured: the project `CLAUDE.md` is 5.0 k tokens of every subagent's first turn and the memory index 6.7 k; the appended system prompt does not reach a subagent (`specs/255-cheaper-checks-by-tooling/research.md`). **The FINDING in this row stands; its CRITERION is no longer the bar.** "Hit every recorded finding" assumes two runs of one check agree, and feature 260 measured that they do not - three runs of `record-format` on one entry agree on eight terms and differ in the tail, in every condition. A scoping change is judged now by whether every candidate `make record-prepass` raised was ruled on in both conditions (`.claude/skills/diagram/research/CLAUDE.md`) |
| **`omitClaudeMd: true` in every defined agent** (feature 256, GM 2026-09-19) | every subagent was handed the root `CLAUDE.md`, the nested ones its reading pulled in and the session's memory index on its first turn and re-read them on every turn after - 47-67 k tokens of fixed context, most of it written for a session and not for a check | one frontmatter line per agent file, after each rule a check leaned on was MOVED into its contract (a rule-by-agent table over all three files). Proven on recorded findings with the agent dispatched as a real subagent: nothing missed on three cases, the first turn down from 55-63 k tokens to 8-12 k, the weight down a quarter. An ad-hoc agent keeps the files. Eight more agents were then paired (ten of twelve in all), each reply scored finding by finding by an independent reader: BOTH legs of most pairs caught what the other missed - a single run of a judging agent is not a stable oracle - and two losses were one-sided: `entry-drift`, repaired by moving the research doctrine's point into its contract (re-run DRIFTED), and `spec-fidelity`, not settled, which keeps the files - the session measured that and recommended it, the GM approved - until a three-samples-a-leg run can tell the field from variance. The tier test enforces the field on every other agent file (`specs/256-defined-subagents-without-claude-md/research.md`) |
...
The others (`discard`, `guard-file`, `repo-safety`, `source-block`, `readme`, `clone-sync`,
...
*CLAUDE.md keeps each in a line; this is the record with the GM's words and the measurements.*

- **Python changes**: `ruff check` + `ruff format --check` + `pyrefly check` (the mypy-strict rule set), and the coverage floor - `coverage report --fail-under=100` over the whole engine, run by a plain `make done`. It is NOT a pytest flag: `pytest-cov` is never given one and `[tool.coverage.report]` deliberately omits `fail_under`, so the floor runs LAST and a run's own failures are reported before the coverage table.
- **100% COVERAGE IS OWED BY EVERYTHING, THE DAY IT LANDS.** *"A new tool absolutely should silently owe one hundred percent coverage the day it lands."* Enforced in the standard manner - a floor on the gate (`coverage report --fail-under=100` in the skill Makefile) with the measured surface DERIVED (`source = ["l7r"]`) - never by a ratchet and never by a roster somebody maintains. **Do NOT put `fail_under` in `[tool.coverage.report]`**: `pytest-cov` reads it there, so it would fire on every partial run (`make test-file`, `make quick`) and would break the deliberate ordering the floor depends on.
- **AN INNER FUNCTION THAT IS HARD TO TEST GETS LIFTED OUT (GM 2026-08-28).** *"If something is only available as an inner function in a closure, then you can move it out into its own function to make it more unit testable. That is a common unit testing paradigm ... you can generally have your unit tests be much simpler if you're just calling functions that take simple inputs and outputs without needing to create a lot of very complicated setup."* **Dropping the test is not one of the options** - a session that meets a closure it cannot reach and writes "dropped (nested closure)" leaves the branch untested. Lift the closure to module level with its captured values as parameters, have the inner one delegate so there is ONE body, and test the lifted function with plain dicts and tuples - `web_pieces`, `web_rejoinable` and `fan_rival` are the worked examples, each of which previously needed a whole settlement, a fabric list and a water list to ask a yes/no question. The exception is narrow and must be stated where it is taken: an inner function that genuinely simplifies the implementation AND whose extraction would cost measured performance. *"There usually is not"* such a benefit.
- **AN OVERLAP CHECK WITHIN A MAP IS PERFORMED IN ITS EFFICIENT FORM (GM 2026-09-08, constitution Principle X clause 15).** *"We will need to take care to make sure we do not have every item on the map checking for overlap with every other item, or anything silly like that."* Any test of a candidate against the features already on the map - a tree against the crops, a scatter mark against the treads and the water, a parcel against the houses - builds the geometry that does not change during the scan into an index ONCE (`settlement/_geom/indexes.py`: a bounding-box prefilter, a `PointGrid`, a `RingIndex`, a `KeepoutGrid` holding every keep-out of one scatter) and asks the index per candidate; the index prunes, the exact test decides, so nothing is coarsened and the maps do not move. The measurement behind it: the windbreak tested each of 37,490 candidate clumps against every edge of every crop polygon (7.2 million segment distances, 77% of a 7.3 s stage) beside an `Indexed` registry the farmhouse placer already queried, and the marsh scatter rebuilt its whole watercourse list per point; both stages went to under 2 s with byte-identical maps. It governs new code and code a profile shows costing time (`make map PROFILE=1`, then `make perf-profile`); a plan that adds an overlap check says how it is indexed. Not mechanically enforced and no subagent reviews it today, by the GM's ruling; the future review of overlap-checking code is recorded in `specs/218-efficient-overlap-checks/spec.md` D4.
- **Edit files with `Edit`, not with heredoc'd Python that rewrites them.** This costs no extra round trips - several `Edit` calls batch into ONE turn exactly like one folded bash command does - and it removes a hazard the patch script carries by construction: Python-inside-a-heredoc that itself contains quoted Python has to be hand-escaped, and when it breaks it breaks as a `SyntaxError` in the PATCHER, so the anchor never even gets tested and a full model turn goes on rewriting the quoting (measured on one fix: three failed patch scripts, about 10% of the task, every one a quoting slip rather than a wrong anchor). Keep heredocs for analysis, asserts, and glue that contains no quoted code - and when an edit genuinely must be scripted (a mechanical sweep over many files), **use `scripts/_patch.py` rather than hand-rolling the loop**: each (anchor, replacement) is its own write, so a stale anchor is REPORTED and skipped while every other edit in the batch still lands, and anchors match whitespace-insensitively because most anchor misses span a line wrap. The ad-hoc form - accumulate every edit, write once at the end - discards the substantive edits beside one cosmetic miss and reports success.
- **Foreground-regenerate ONLY the motivating map - never a pre-gate pool sweep.** The gate verifies the pool itself, and render-sync regenerates main's renders from main's own tip, so a clone-side `pipeline/regen.py pool/*/*/*.gen.py` before `make done` is pure waste (evidence in [`iteration-loop.md`](iteration-loop.md)).
- **EVERYTHING RUNS THROUGH `make`, and it is enforced rather than requested.** A bare interpreter reaching an engine entry point, a bare pytest, a make driven by a foreign makefile, or an inline override supplied to skip a prompt is refused BEFORE it runs by `scripts/make-only-hooks.sh`; the engine refuses in-process calls too. In the diagram skill the ladder, cheapest first, is `make quick`, `make done` (NOT the quick one), `make done FULL=1` (prompts, cancels by default, logs a reason). **What each costs is asked of the recorded runs, never written down** (GM 2026-08-30, on a guard message quoting a gate cost that had been wrong for three days): `make audit` prints the history; `scripts/_gatecost.py <target>` prints the median a guard message would quote. Every refusal names the target that does the job, because a guard that blocks a legitimate question without giving the route is a guard that gets worked around.
- **`make quick` selects by CHANGE** (pytest-testmon, GM 2026-08-26): it runs only the tests whose executed code changed since the last run - nothing changed means "no tests ran" in a few seconds, a one-line engine edit runs its handful of tests. `make quick ALL=1` runs every quick test; the gate never selects. A test that reads a DATA file (a fixture, a manifest) re-runs only when its code changes - the gate covers that. **A red `make quick` saying "Different tests were collected between gw0 and gwN" was testmon, not the test** (2026-09-19; six recorded occurrences since 2026-09-05, each the first run after a test file gained an id, each green on an unchanged re-run): testmon 2.2.0's controller wrote the collected ids into `.testmondata` as soon as its ready/finished counter touched zero, which a fast worker reaches while slow ones are still reading, and a worker whose two reads straddled that write deselected the new ids. `tests/_testmon_sync.py` replaces that sync with one that waits for every worker; the mechanism, the forced reproduction and the ordering trap (the conftest hook is `trylast`) are recorded there. If the message ever returns, read it - do not re-run past it.
