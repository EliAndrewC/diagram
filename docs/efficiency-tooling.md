...
| **`omitClaudeMd: true` in every defined agent** (feature 256, GM 2026-09-19) | every subagent was handed the root `CLAUDE.md`, the nested ones its reading pulled in and the session's memory index on its first turn and re-read them on every turn after - 47-67 k tokens of fixed context, most of it written for a session and not for a check | one frontmatter line per agent file, after each rule a check leaned on was MOVED into its contract (a rule-by-agent table over all three files). Proven on recorded findings with the agent dispatched as a real subagent: nothing missed on three cases, the first turn down from 55-63 k tokens to 8-12 k, the weight down a quarter. An ad-hoc agent keeps the files (`specs/256-defined-subagents-without-claude-md/research.md`) |
