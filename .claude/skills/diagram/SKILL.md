...
- [`tests/gate/`](tests/gate/) - the rules that used to be the check battery, asserted once per code change on a cached roll instead of once per map generated. Each states what it FOUND before it states the found thing is well formed, because a test whose subject list is empty passes
...
- [`pyproject.toml`](pyproject.toml) - pytest + coverage config. Coverage is **100% over the whole MEASURED set, enforced on a plain `make done`** (feature 174, 2026-08-31) - 20,682 statements, 0 missing. "Measured set" is `[tool.coverage.run] source` in `pyproject.toml`, which names the measured modules one by one and leaves 19 engine files (~1,843 statements of `tools/` audits and drivers) outside, deliberately, so a new tool does not owe 100% the day it lands. The 2026-08-16 legacy freeze's per-module arrangement - 100% everywhere except a 94% ratchet on `settlement/`, whose above-hamlet wings nothing exercised - retired when those wings were covered by tests instead of by waiting for the tier conversions. Run `python3 -m pytest` from the skill dir (needs `pytest` + `pytest-cov`); the full suite must run together or the coverage numbers are meaningless
