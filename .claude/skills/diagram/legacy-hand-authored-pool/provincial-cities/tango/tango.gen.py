...
doctrine - see research/cities/sizing.html). The placement is bounded by the wall (s.bound), so the dense
...
from l7r.diagram.citybudget import CityProgram, budget_to_manifest, plan_city  # noqa: E402
...
s.city_wall(
    WALL, gates=[NGATE, SGATE], guard_east=[NGATE], tower_skip=[(2065, 1364), (1655, 1748)]
)  # the fence-END kido spots (E + SW ring-road gates): the gates keep their crossings, the towers slide a short way along the wall
...
# realistic, not an omission. See research/water.html 'Does a moat have a current?' for the full grounding.
...
# absorb a live stream); see research/water.html 'Does a moat have a current?'. Threads S between the Imperial road
...
# TRADE WORKS (GM 2026-07-24, research/urban-features.html "Trade works - which trades outgrow the shop glyph, and why"): the
...
def comb_field(name, sluice, down_deg, seed, field_fall, canal_a, canal_b, offtakes_a, offtakes_b=(), dry_band=(14, 26), avoid=(), mirror_ym=None, dry_keepout=(), inwall_drain_bias=None):
    ...
    for dp in net["dry_plots"]:
        ...
        if any(hem_on_paddy(dp["poly"], _pol) for _pol in _prior_paddies):
            continue  # hem plot lands on a NEIGHBORING fan's rice (fans are placed blind to each other; the fe2-into-fe1 incident) - dry_plots_clear_of_paddies
...
VEG_CROPS = {"daikon": ("#9FB86B", "#83A050"), "greens": ("#8FAE62", "#75954C"), "onions": ("#ADBC77", "#93A25C"), "beans": ("#A9B36A", "#8E9A50")}


def veg_tract(name, bbox, seed):
    """An in-wall kind="vegetable" GARDEN TRACT (research/cities/hinterland.html 'Does a city farm inside its walls?' ('In-wall VEGETABLE tracts'):
    ...
    for i in range(len(rows) - 1):
        ...
        for j in range(len(cuts) - 1):
            quad = [
                (cuts[j] + R.uniform(-2, 2), rows[i] + R.uniform(-2, 2)),
            ...
            s.M["dry_plots"].append({"poly": [[round(x, 1), round(y, 1)] for x, y in quad], "crop": prev, "theta": round(theta, 3)})
            s.dry_polys.append(quad)
    outline = [(x0, y0), (x1, y0), (x1, y1), (x0, y1)]
    s.field_polys.append(outline)
    s.M["fields"].append({"name": name, "kind": "vegetable", "outline": [[x, y] for x, y in outline], "bbox": [x0, y0, x1, y1], "vis_bbox": [x0, y0, x1, y1]})
...
BUR_ST = [
    [(1191, 1285), (1602, 1285)],
    [(1480, 1233), (1480, 1371)],
]  # the vertical runs a full 50+ past the lane both ways (no dangling stub) and into the SW street   # the vertical continues into the SW x1480 street (end-to-end junction, no near-miss)
grid(BUR_ST)
...
for wx, wy in [
    ...
    (1412, 1376),
    (1344, 1420),
]:  # (1412,1376)+(1350,1408) added 2026-07-21: the torii-reservation repack left that court's draw-point serving 27 households (city_well_density_sufficient)
    s.well_at(wx, wy)
...
s.frontage(
    [(1602, 1145), (1967, 1145)], ["shop"] * 3 + ["laborer_large"] * 12, width=s.lw(18), spacing=17, rows=1, rowgap=2, jitter=1, setback=s.px(14), skip=NE_ST[0]
)  # stops short of the ring arc (a large corner was reaching the patrol bed)
...
for fan in (
    [(1817, 1046), (1813, 1023), (1821, 1062), (1882, 1021), (1880, 1058)],
    ...
    [(1742, 1272), (1786, 1272), (1748, 1258)],
):  # the last fan splits the (1766,1245) draw-point the roji cadence pushed to 27 households (city_well_density_sufficient); candidates sit a court SW of it - anything nearer is inside the seam well's own berth
    for c in fan:
        if s.well_at(*c):
...
# corner of the road x main-street crossing (research/urban-features.html "The bell-and-drum tower - one per walled seat"); placed
...
)  # ONE beside-hall gate (was 2 - torii_count_canonical numerology; a third up the street mis-attributed to Benten, and the S street end is packed, so the modest 1-gate entrance it historically had)
s.ministry(
    1308, 1608, "Ministry of Rites", w=s.px(140), h=s.px(95)
)  # nudged SW 2026-07-21: the torii-reservation repack tilted a merchant_large against its old NE margin (city_government_offices_dont_abut, rotation-aware gap)
...
s.place_wells(
    (1181, 1338, 1602, 1704), spacing=47, near=46
)  # merchant district + temple neighborhood (spacing 52->47, 2026-07-21: the true-scale reflow packed the avenue rows denser and the (1412,1376) well sat nearest for 33-34 households with every interior seed spot reserved - a finer grid is the honest fix, ~1 well per 10-20 households)
...
for _wc in [
    ...
    (1255, 1455),
    (1250, 1480),
]:  # a west idobata for the temple-hood merchants: the (1412,1376) well's 34-household catchment reaches to x1267, and every interior spot is reserved (hall block / label bands) - this splits off its west wing
    if s.well_at(*_wc):
        break
for _wc in [
    (1620, 1452),
    ...
    (1550, 1520),
    (1560, 1540),
]:  # a seeded idobata for the merchant district's SE pocket by the ward fence: the true-scale reflow (2026-07-21, merged engines) left its densest well nearest for 30+ households (city_well_density_sufficient ceiling 26); candidate LIST per the well_at doctrine - blocked spots no-op
    if s.well_at(*_wc):
        break
...
MIN_POS = [
    ...
    (1941, 1419),
    (1957, 1484),
]  # staggered rows between fence and avenue; the upper row labels ABOVE (over the bare fence line), the lower row + Justice label BELOW (into their own block margins)
s.block_polys.append(
...
_mp2 = moat_swept_tap(MOAT, s.M["moat_flow"]["inlet"], s.M["moat_flow"]["outlet"], _sl2, _mp2)  # sweep the throat downstream; _sl2 (and so fe2's field) does not move
...
s.place_caption("gate market", s.frontage_box, 10)  # seated by the standoff ladder against the stall row's own extent
s.frontage(
...
s.place_caption("gate market", s.frontage_box, 10)  # ditto - the ladder also keeps it clear of the Imperial Road caption
...
)  # parish=False -> ORGANIC Japan-style plot (research/religion-and-death.html 'Burial ground shape - Japan organic, China surveyed' ('shape of the common ground'). Back inboard for the content crop (2026-07-23 second pass): the funerary anchors the frame, so it sits just past the fields, not at the old wide frame's edge
...
# OUTLINE, the fan's unplanted rim slack counted as water too and the pair genuinely had nowhere
# legal to drink: a probe found 150 of 168 ring seats refused. The hand seat was correct THEN.
#
# It stopped being needed when the rule moved to the DRAWN basins (`paddy_wet_rings`), which handed
# the rim slack back. Nobody re-tested the premise, so the seat and its rationale outlived the
# problem by two weeks. Re-measured 2026-08-08 at the moment farm_wells returns: both steadings come
# out COVERED on farm_wells' own wells, at 450 ft and 354 ft against the 500 ft reach. Removing the
# seat and regenerating leaves farm_wells_within_reach PASSING and the whole gate output byte-
# identical to the version that had it. The well was doing nothing but standing there.
#
# The rationale it carried was wrong twice over, which is the lesson worth keeping. It blamed
# `_fits`' circumscribed circles (skill CLAUDE.md, "CENTER vs FOOTPRINT", item 2) for leaving the
# pair dry; an attribution probe over the 8,065 seats that could cover both steadings put the circle
# clause at 5.7% of refusals here, well behind paddy water (26.9%), corridors (23.2%), dry plots
# (16.5%) and block polys (14.5%).
# It also described farm_wells as trying "six dooryard offsets", which is not what the code does -
# it walks 8 rings of 8-24 points per steading and then a ~2,700-point 6px lattice.
#
# THE RULE THIS LEAVES BEHIND: a hand seat is adjudicated against the gate at the moment it is
# written, and that adjudication is a snapshot, not a standing fact. When you change the rule a
# hand seat was working around, delete the seat and re-run the gate before assuming it is still
# load-bearing.

# ===== FIRE DEFENSE: watch-towers =====
# ONE AMID EACH major commoner quarter, on the cleared seams the dense city leaves - placed
# LAST so they perturb nothing. WHY: research/cities/fabric.html "How did a dense wooden city watch for fire?".
...
def _swap_halo(cx, cy, w, h, old_m=26, new_m=16):
    for _i, _p in enumerate(s.block_polys):
        xs = [q[0] for q in _p]
        ys = [q[1] for q in _p]
        if abs(min(xs) - (cx - w / 2 - old_m)) < 0.6 and abs(max(xs) - (cx + w / 2 + old_m)) < 0.6 and abs(min(ys) - (cy - h / 2 - old_m)) < 0.6 and abs(max(ys) - (cy + h / 2 + old_m)) < 0.6:
            s.block_polys[_i] = [(cx - w / 2 - new_m, cy - h / 2 - new_m), (cx + w / 2 + new_m, cy - h / 2 - new_m), (cx + w / 2 + new_m, cy + h / 2 + new_m), (cx - w / 2 - new_m, cy + h / 2 + new_m)]
            return True
    return False
...
top_up(
    "samurai", (1639, 1392, 2067, 1730), 54, count_kinds=("samurai", "samurai_large")
)  # constrained to the SE QUARTER WEDGE'S interior: the wider sweep above seats houses along the E arc OUTSIDE the quarter's inset-24 polygon, which the per-quarter density counter cannot credit - only in-wedge seats lift the quarter over the 0.30 residential floor (city_residential_quarters_dense_enough)
top_up(
    "merchant_house", (1181, 1350, 1594, 1712), 120, count_kinds=("merchant", "merchant_house", "merchant_large")
)  # 120 (was 112), region grown W to the row band's own x1181 edge + S to the ring taper: the pair cadence dropped the merchant caste to 102 vs its 105 band floor (~150 target, city_caste_counts_in_band); the finer 6x7 exact sweep finds the SW district's residual seats
top_up(
...
for _kind, _region, _cap in (
    (
        ...
        (1201, 1226, 1597, 1344),
        155,
    ),  # the open W-central / burakumin-quarter band (servants interleave among the commoners) - no other servant region reached it, leaving it under-filled (feature 006)
    ...
    ("merchant_house", (1194, 1350, 1594, 1701), 120),
):  # merchant band headroom (caste max ~195) absorbs any residue
    _dw = _dwell_count()
    if _dw >= 562:
        break
...
print("farmhouses:", len(s.M["houses"]), "| buildings:", nb, "| total urban:", sum(nb.values()), "| estates:", len(s.M["manors"]), "| finish:", s.finish(os.path.join(HERE, "tango")))
