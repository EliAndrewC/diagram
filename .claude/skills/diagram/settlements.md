...
This file is the Mode B half of the /diagram skill: settlement maps built by the parametric generator (the [`settlement/`](l7r/diagram/settlement/) package) and gated by the validator ([`check_village/`](l7r/diagram/check_village/)).  Read [`SKILL.md`](SKILL.md) first for everything shared (principles, workflow, labeling, style conventions, render pipeline, output convention).
...
| [`settlements/capitals.md`](settlements/capitals.md) | The subject is a domain capital (`meta(scale="capital")`) - the daimyo's castle-town seat, ~12,360. Its **space budget and declared knobs are SHIPPED** (feature 018: `citybudget.plan_capital`, `--tier capital`, and the two budget checks); every DRAWN feature is still a design record pending feature 019. A capital inherits the whole provincial-city tier and overrides it. |
...
Code comments (the `check_village/` package especially) point at sections by name, in the form `settlements.md "NAME"`.  Those pointers still land here; this table forwards them:
...
The common machinery lives in the [`settlement/`](l7r/diagram/settlement/) package (the `Settlement` class; subfile index in [`settlement/CLAUDE.md`](l7r/diagram/settlement/CLAUDE.md)). A specific settlement is a **thin spec** that imports it, declares its fields/water/shrine/houses, and calls `finish()`. Worked examples, smallest to largest (the town examples are detailed in [`settlements/towns.md`](settlements/towns.md), the city ones in [`settlements/cities.md`](settlements/cities.md)):
...
- [`legacy-hand-authored-pool/towns/hoshizora/hoshizora.gen.py`](legacy-hand-authored-pool/towns/hoshizora/hoshizora.gen.py) - an **unwalled town** (county seat, ~1,200 people): Imperial Road spine, road-fronting urban core, all castes, water-first combs wedged between stream and road.  Full detail in [`settlements/towns.md`](settlements/towns.md).
- [`legacy-hand-authored-pool/towns/hirameki/hirameki.gen.py`](legacy-hand-authored-pool/towns/hirameki/hirameki.gen.py) - a **walled town**: hill-anchored rampart, gate-to-yamen streets, chrysanthemum field, two monasteries, five combs covering every legal water topology.  Full detail in [`settlements/towns.md`](settlements/towns.md).
...
2. Write a thin spec by copying the closest existing one: a **village** from `kikuta.gen.py` (pond, one comb-field, communal windbreak) or `hikari-no-sato.gen.py` (stream, split two-block, per-house groves); a **hamlet** from `moritono.gen.py`; an **unwalled town** from `hoshizora.gen.py`; a **walled town** from `hirameki.gen.py`. Running it writes the `.svg`, the `.json`, **and the `.png`** next to itself - `s.finish()` rasterizes the PNG automatically (via `resvg` at 2600px), so the `.png` can never drift from the `.svg`. Only touch the `settlement/` library if you need a genuinely new shared capability.
3. Run `python3 -m l7r.diagram.check_village <manifest>.json` - the machine gate. It must report ALL CHECKS PASSED before going further. (Running the test suite, `python3 -m pytest`, re-runs every gen and so refreshes every pool `.json` and `.svg` - but the suite sets `DIAGRAM_SKIP_RENDER=1`, so `.png` files only refresh when you run a gen directly. If you ever hand-edit an SVG, re-run the gen rather than calling `resvg` yourself. Renders are gitignored and reach main via the stop-work procedure's render-sync step in CLAUDE.md - the GM browses the complete render set in main's pool dirs.)
...
Pull from `/host-l7r-repo/gm-assistant/setting/median-domain.md`, `demographics.md`, `village-headsmen.md`:
...
- **What a settlement's page STATES about its size, per tier** (GM 2026-08-29; the underlying
  ...
  **farmhouse count**, exact and untilded - every household is drawn, so the reader could count them -
  and a population of ~5 x households. A town and a city state their **non-farm dwelling count**
  instead, also exact, and **no farmhouse count at all**: the GM's reason is that both *"are
  surrounded by significant farm fields and farmer populations, which are deliberately not all
  rendered on the map. Therefore, that number should not be included."* Their populations are then
...
### The validator (the `check_village/` package)
...
1. **Any rule that can be machine-checked belongs in the `check_village/` package, not only in this prose (its CLAUDE.md says which file).** When a new Mode B rule emerges, add it as an assertion.
