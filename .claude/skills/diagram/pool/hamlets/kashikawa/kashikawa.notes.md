...
returning to the brook at a confluence inside the view.
...
- **The notice board is no longer at the traffic optimum and these notes said it was.** Measured
  2026-08-29 and RE-MEASURED on the roll that ships (the first figures here, 9 of 20 at
  (2255.5, 2989.2), were themselves a roll stale by the time the commit carrying them landed - which is
  why the board's position and reach now live in the Census block below rather than in this sentence).
  The board stands short of the best available placement anywhere in the cluster. The feature-145 entry
  records "the board sits at the traffic optimum (14 of a possible 15 dwellings within 250 ft)", which
  described a roll that no longer exists. The board sat
  at (1999.8, 2852.0) with 13 of 13 at `b248ab25` and moved at `8260a6e0` - the
...
**The confluence reads as water again.** This is the pool's one map of a drain returning to its brook, and pass 10's
drain hue - chosen because the first one was indistinguishable from the supply - had left the water family altogether at
hue 171: against the tan hinterland and the brown bund the collector hems for its whole length, it read as a second
boundary line, and the junction read as the field's edge touching a stream rather than as the village's water going
back. The separation is kept on lightness and saturation and given back on hue (`#4F7186`, 203 degrees, value 0.53
against the supply's 0.75, 76 RGB units from it where the rejected first hue stood 21).

**The title no longer has to miss its own belt's outline.** A grove was an obstacle by its recorded polygon - the band
its clumps were seated in - rather than by the canopy it drew; it blocks where its trees are now.
...
- windbreak: **194** clumps drawn, **63** off the page
- copse: **70** clumps drawn
- farmhouses: **20**
- farmstead fixtures: bath **12**, coop **20**, pit **6**, privy **16**, shrine **1**, woodpile **15**
- notice board at **(2364.5, 3166.9)**, **6** of 20 farmhouses within 250 ft
<!-- /census -->
...
the caption is drawn rotated about its center (here -42.7 degrees), so the discs march along a line
...
**What moved on this map: the notice board's caption box, by 2.0 px, and nothing else.** A
...
map's caption was already seated centrally - its along-baseline offset is 1.53 px against a
...
culls the blades wholly outside the frame plus 24 px before merging, so the SVG holds 30,558 blade subpaths where it
held 361,086; the page's own subpath count did not move, since the page had dropped them itself). settlement-review
...
water, 0 inside the cluster's hull - before and after; in-view density per 1,000 sq ft blades 5.43 -> 5.48, reeds 3.71 -> 3.65; the
...
## 2026-09-12 (feature 226): the homesteads seated from the site boundary - the layout moved
  ...
  On this map the change is the largest of the pool: 20 of 20 seated on the first roll with 4.8 candidates per
  house (1.4 placer calls), the elongated cluster honored at a drawn aspect of 3.92 (3.08 before). The
  review's first pass, on the cut before the toe joined the boundary, found two things this entry records: the
  cluster had stretched to 4.96 and carried house 10 into the reed-marsh toe (wholly, 46 px deep - the record,
  not the ink), and 7 of its 22 garden beds had a 10 m belt clump 24-38 ft south of them with no rule firing.
  The toe is in the boundary now and the belt's southern strip is 39 ft: 0 footprints in the toe, 0 beds
  shaded from the south (2 of 26 before this feature). The boundary is 10 chords, three rings, 5 water and 3
  corridor segments. Two of the review's questions are recorded rather than acted on, both pre-existing
...
## 2026-09-12 (feature 227): the homesteads seated envelope first - the layout moved
  ...
  On this map: 20 of 20 seated (roll attempt 1), 27 placer calls at 4.2 envelope tests each and
  11 rectangle tests per house against 220 under feature 226; the front row seated 9 and the ranks
  behind ran 2 rounds. The nearest house stands 47 px from the field outline with 12 within 165, and
  homestead to homestead the median gap is 8 px (worst 32) - the fabric is continuous, which a center-to-center
  reading of the same cluster does not show. Drawn aspect 2.79 (elongated, unhonored); windbreak 240 clumps,
  copse 108; 3 of 20 gardens split, sides {'E': 15, 'W': 2, 'S': 3}.

## 2026-09-12 (feature 227, D11): the straggler arrives at the dooryard, and the rule now says so

  This map is where the defect was found. Its straggler footpath - the only way to house 3, the outlying
  steading at (1773.7, 3037.9) - ends at (1829, 3068), which `lanes_reach_something` read as a tread
  stopping in open ground: 63 ft to the nearest house CENTER against a 60 ft bar. The measurement says
  otherwise. The end stands 7.5 ft from that steading's own garden bed, 26 ft from its byre and 28 ft
  from the farmhouse's wall, and on the render there are about 4 ft of bare parchment between the drawn
  fence and the drawn tread - less than the width of the tread itself. That is where the clip leaves a
  footpath by design: `clear_runs` cuts at the fabric margin and the steading's own yard is in the
  obstacle set, because the lane ends AT the yard and the yard is ground the household crosses on foot.
  What was wrong was the measurement: a 46x28 ft farmhouse carries 27 ft of itself between its center and
  its front corner. The rule gained a fourth clause at `STEADING_ARRIVAL_FT` (12 ft, derived from that
  margin plus the 4 ft step the clip walks in) for a steading's BUILT ground - house, byre, shed, yard,
  garden - with the grazing commons and the groves deliberately left out of it, since a tread that stops
  in them has stopped in a field. The map's own layout did not move for this; its lane records are the
  envelope-first re-pack's.

  MEASURED AND ACCEPTED, so the next session does not re-open it: thirteen free ends across the five
  shipped hamlets pass the rule only through its "within 60 ft of another way" clause, and every one of
  them stands 0.0-0.1 ft from another lane - they are junctions, which is what that clause is for. The
  settlement-review asked whether an end that OVERSHOOTS its own junction would pass by construction,
  since the clause cannot tell a way the end reached from the way it just left. It would - and no such
  end exists on the pool today: the 11.5 ft stub at (2295.9, 2609.0) that prompted the question passes on
  its own steading's house center, 14.5 ft from that wall. So the clause is left as it is rather than
  tightened against a case nothing produces. The alternative priced and declined: measure the way clause
  against a way the end did not come from, which would trim legitimate short spurs off a junction for no
  measured gain.

  The same review asked whether the regular brick seat lattice aliases the position-seeded prevalence
  shares, having measured this map's kura at 3 of 20 against a documented ~30%. Measured pool-wide: 23 of
  82, 28%, with the five maps at 27/15/31/25/42% - the pool sits on the number and this map's roll is the
  low draw in it. Chance, not aliasing.
...
tap, near the bottom-left of the sheet, is a brook and a ditch parting, which is what was actually there; the
junction near the top is the other end of the same water, the drain coming back to the brook.
