...
`build_comb`), and a windward quarter derived from the slope rather than declared regionally. The note that
a hamlet named for its sluice drew no glyph at its intake is closed: since feature 230 the brook is tapped at an
intake on its bank and runs on past the field, and this map draws a weir there.
...
**The drain stopped arching over its own pond.** Pass 10 led the outfall round a curve where the pond lies behind the
collector's heading, and threw the curve's handle 0.6 of the distance ALONG that heading - so with the pond 127 degrees
round, the ditch climbed 26 ft further from it, topped out 53 ft past its far rim and came back down: 121 ft of ditch on
an 87 ft chord, an inverted U whose apex passed 11 ft from the brook it does not join, posing a junction this map does
not make. Every per-bend angle was inside tolerance, which is why nothing caught it - a per-bend limit cannot see an
excursion. The handle points along the bisector of the heading and the chord now: 7% detour, sharpest bend 14.9 degrees,
...
<<<<<<< HEAD
- windbreak: **170** clumps drawn, **24** off the page
- copse: **18** clumps drawn
- farmhouses: **12**
- farmstead fixtures: bath **4**, coop **10**, pit **5**, privy **10**, shrine **1**, woodpile **12**
- notice board at **(791.8, 1957.1)**, **5** of 12 farmhouses within 250 ft
=======
- windbreak: **137** clumps drawn, **2** off the page
...
## 2026-09-12 (feature 227): the homesteads seated envelope first - the layout moved
  ...
  On this map: 12 of 12 seated (roll attempt 1), 15 placer calls at 3.5 envelope tests each and
  9 rectangle tests per house against 113 under feature 226; the front row seated 6 and the ranks
  behind ran 2 rounds. The nearest house stands 37 px from the field outline with 7 within 165, and
  homestead to homestead the median gap is 19 px (worst 68) - the fabric is continuous, which a center-to-center
  reading of the same cluster does not show. Drawn aspect 2.31 (round, unhonored); windbreak 168 clumps,
  copse 20; 2 of 12 gardens split, sides {'S': 2, 'W': 4, 'E': 6}.
=======
