...
- **2026-08-17, the three consequences of true size** (GM: *"Yes please tackle those now"*, on the
  ...
  **Planks only where you cannot stride** - CURRENT: **10 planks, `FOOTPLANK_MIN_FT` = 2.0 ft**
  (GM ruling 2026-08-26, feature 133 T11: the GM asked why the delivery ditches carried no planks -
  *"They don't look too narrow"* - the research pass put the traditional farm ditch at 2 shaku,
...
|---|---|
| the belt "carries 194 clumps" | Both halves moved under feature 152 T02: the belt's trim now tests the real page rather than a proxy for it, so `clumps` finally means canopy a reader can see. **The count itself is in the Census block below** - it was re-typed here once and went stale again within the same feature. |
| "THREE stands" of woodland | **one**, 63 crowns |
| privy 12, woodpile 7, coop 7, manure 5, bath 3 | The privy count rose under feature 152 T07 - the outhouse now has a sun-side seat to go to, so far fewer are refused outright. **The counts are in the Census block below**, for the same reason as the row above. |
...
**The windbreak had thrown a limb into the grazing.** The belt's near face is sampled in columns across the wind, and a
column holding no house of its own leaned on the whole cluster's windward-most house - which is harmless on a cluster
lying square to the wind and, on the diagonal ribbon this feature's seat rule produced here (aspect 5.01), is the far
end of the settlement. The last column jumped 437 ft over 99 ft of x and 32 clumps drew a 430 ft file of trees along the
frame, 246-522 ft from any farmhouse. Every adjacency check passed, because the belt's other 222 clumps are adjacent to
the houses. A column with no house now leans on its nearest neighbor that has one: furthest clump 522 -> 263 ft, none
past 300.

**The notice board stands in the open.** Its caption has kept clear of grove clumps since August; the board itself never
did, so the plank could be buried while its name floated clear. Both seat searches now prefer open ground, and this
board sits 33.2 ft clear of the nearest crown.

**And the map's one caption faces what the gate measures.** The board sits 9.0 ft from BOTH arms of a lane's corner -
exactly equidistant - and the siter broke that tie by the smaller bearing while the check broke it by manifest order,
so the same plank was square-on to its way and 64 degrees side-on to it depending which was asked. One body answers now,
and the tie-break is part of the rule: first in manifest order.
...
- windbreak: **262** clumps drawn, **0** off the page
- copse: **47** clumps drawn
- farmhouses: **15**
- farmstead fixtures: bath **6**, coop **11**, manure **9**, privy **15**, shrine **1**, woodpile **13**
- notice board at **(2633.3, 771.4)**, **4** of 15 farmhouses within 250 ft
<!-- /census -->
...
**This map's outcome**: rolled `entrance`; the board stands at (1097.5, 778.4) with 8 of
...
culls the blades wholly outside the frame plus 24 px before merging, so the SVG holds 49,796 blade subpaths where it
held 260,309; the page's own subpath count did not move, since the page had dropped them itself). settlement-review
(2026-09-11) diffed the render against the previous one: 6 px of 6.8 million, max channel delta 1; the scrub and reed ink in the outer 36 px band is pixel
...
## 2026-09-12 (feature 226): the homesteads seated from the site boundary - the layout moved
  ...
  On this map: 15 of 15 seated on the first roll with 4.3 candidates per house (1.5 placer calls); the
  crescent honored at a drawn aspect of 3.25 (2.86 before); 12 houses within 165 px of the paddy against 10.
  The boundary is 7 chords, three outline rings, 5 water and 3 corridor segments. The settlement-review's
...
## 2026-09-12 (feature 227): the homesteads seated envelope first - the layout moved
  ...
  On this map: 15 of 15 seated (roll attempt 1), 21 placer calls at 2.6 envelope tests each and
  7 rectangle tests per house against 194 under feature 226; the front row seated 7 and the ranks
  behind ran 2 rounds. The nearest house stands 44 px from the field outline with 7 within 165, and
  homestead to homestead the median gap is 31 px (worst 106) - the fabric is continuous, which a center-to-center
  reading of the same cluster does not show. Drawn aspect 2.49 (crescent, honored); windbreak 197 clumps,
  copse 88; 2 of 15 gardens split, sides {'W': 10, 'S': 2, 'E': 3}.

## 2026-09-12 (feature 227, D11): a lane end is judged against a wall, and one arm runs to the byre

  `lanes_reach_something` asks that every internal lane end come within `WAY_END_REACH_FT` (60 ft) of another way, a
  farmhouse or the field, and it measured a farmhouse by its CENTER - which is inside the house, not where anybody
  walks to. A 46x28 ft farmhouse carries 27 ft of itself between its center and its front corner, so a tread that
  stops at a steading's garden fence scored 63-76 ft and read as a line ending in grass (the two that did were
  Kashikawa's and Kuwabata's). The rule gained a fourth clause at its own much tighter distance
  (`STEADING_ARRIVAL_FT`, 12 ft, derived from the margin the clip leaves and the 4 ft step it walks in): an end
  standing that near a steading's BUILT ground - house, byre, shed, threshing yard, garden - has ARRIVED there. The
  grazing commons and the homestead groves are deliberately not in that set: they are what the ground IS, so a tread
  that stops in them is still caught. The placer's trim and the check read one body now (`end_serves`), and the two
  trim callers that had kept a looser bar - the straggler pass and the late tidy-up - ask the gate's own 60.

  On this map, four free ends (the rest are junctions), every one of them at something a reader can see: (1070,1231)
  **7.6 ft off byre 1**, (1229,1311) 12.0 ft off a woodpile and 12.4 ft off house 5's wall, (1248,599) 5.5 ft off a
  garden bed and 9.6 ft off a threshing yard, and the connector leaving the frame to the northwest, which is the
  off-map convention. The byre arm is the change a reader can see: 252 ft due south through the commons between the
  copse and the west rank, stopping short of the byre's north face and 6 ft off its centerline - a tread worn to the
  byre door, which the center reading had been trimming off. The two other ends were trimmed as before.

  Measured and left, so the next re-pack has the numbers: the west rank (houses 10-12) stands 76-81 ft from any tread
  against a 100 ft rule (65 ft was the worst at this feature's merge base, 79 the worst ever shipped, feature 123);
  8 of 10 interior web vertices now turn 60-102 degrees against 5 of 16 before, which `lanes_bend_like_paths` does
  not measure (it fires at 140); and the notice board's audience fell to 2 of 15 dwellings within 250 ft, from 6,
  because the entrance anchor follows the outermost farmstead and the re-pack made house 14 an outlier 246 ft from
  its nearest neighbor. All three are the settlement-review's findings on the re-roll, none is a rule breach, and all
  three are the GM's to rule on at acceptance.
...
among the rest. The strike-out has one limit, found by measuring a 48-seed cohort against the pre-feature
