...
| 13 | `stage_crossings` | planks and decks over every way that crosses water; on a polder the ring-canal planks cluster on the settlement-side toe collector and skip the feeder, the far toe and the drain (`polder_crossing_caps`, feature 139) |
...
**And its FEATURE CLASS (feature 139).** Every `add*()` at the glyph's emit site carries the class
