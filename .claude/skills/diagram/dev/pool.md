...
The pool is two sibling trees, each `<tree>/<tier>/<map>/` - one folder per map, holding that
map's whole bundle (`<map>.gen.py`, `.json`, `.notes.md`, and its renders).

| | `pool/` | `legacy-hand-authored-pool/` |
|---|---|---|
| holds | the LIVE maps: 5 scripted hamlets + 5 Mode A magistracies | the 18 FROZEN hand-authored Mode B exhibits |
| regenerated / re-gated | every run | **never** |
| renders in git | derived, ignored (a Mode A `.svg` is the exception - it IS the source) | **committed write-once** |
| `poolmaps.classify()` | `scripted` or `compound` | `legacy` |
...
rules are per-tree patterns rather than per-file exemptions. Nothing else to add or delete.

`pool/regressions/` stays FLAT inside the live tree: 107 negative fixtures, one file each, no map
bundle to fold and no index row.

**Everything that walks the pool asks `poolmaps.bundles()`**, saying which tree(s) its job concerns
(`contracts/pool-discovery.md` in `specs/161-pool-per-map-folders/` has the per-consumer table). Do
not write a new glob: ten consumers used to hardcode the shape independently and they drifted -
`mapcheck._live_gens` records Kuwabata being regenerated by `regen.py` while `make maps` never
rolled it. The tree argument is the load-bearing one, because getting it wrong is SILENT: a sweep
over too few maps is green.

## The legacy pool is FROZEN (GM 2026-08-16)

Every hand-authored Mode B map - 8 hamlets, 4 villages, 3 towns, 3 provincial cities - is a
permanent EXHIBIT: never regenerated, never re-gated. They stay in `pool/index.html` (which covers
both trees on one page) and their committed .json/.svg/.png stay exactly as shipped. `pipeline/poolmaps.py` is the classification
...
- **The renders are COMMITTED, write-once** (GM 2026-08-16): a frozen exhibit's svg/png cannot be
  faithfully re-derived once the engine drifts, so the 18 maps' renders are tracked in git - the
  one exception to "pool renders are ignored" - expressed since feature 161 as a per-TREE rule
  (nothing under `legacy-hand-authored-pool/` is ignored) rather than the 36 per-file `!` lines it
  replaced. When a map is CONVERTED to the scripted approach, `git mv` its folder into `pool/`: the
  converted map's renders are
...
`settlement-review` agent was reading `pool/hamlets/inashiro/inashiro.json` at the time, and the stash
