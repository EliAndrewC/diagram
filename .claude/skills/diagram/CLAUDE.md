...
| `make done` | reference + lint/types + the whole suite + coverage floors. **Short-circuits in seconds** (`already verified`) when the last record is a green `make done` against exactly this gate content - every `.py` under the skill, tests, pool gens, Makefile, pyproject, lockfiles, `scripts/` (feature 132; [`dev/switches.md`](dev/switches.md)). Docs-only edits never cost the gate | **~5.5 min - NOT the quick check** (or ~3 s already verified) |
