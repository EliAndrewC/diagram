...
| `make scope-lock REASON=...` / `scope-unlock` | **scope locked**: no invocation rolls a map other than the reference settlement or more than one map - `cohort`, `test-full`, `done FULL=1`, `cache-audit`, `perf`, `maps SCOPE=all` and any remote run refuse and name the unlock. (`tripwire` was retired 2026-09-05 and `regressions` by feature 166; a live instruction must not name a target that no longer resolves.) Commits the switch. Doctrine: [`dev/switches.md`](dev/switches.md) | ~1 s |
