...
| `make done` | reference + lint/types + the whole suite + coverage floors. **Short-circuits in seconds** (`already verified`) when the last record is a green `make done` against exactly this engine content - the dispatcher's own keys: every `.py` under the skill, plus tests and pool gens/manifests (feature 132; [`dev/switches.md`](dev/switches.md)). Docs, Makefile, config and `scripts/` edits never cost the gate | **~5.5 min - NOT the quick check** (or ~3 s already verified) |
