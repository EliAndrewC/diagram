...
**Sources:** `religion-song-enwiki` (READ: 1221, 400,000 monks and 61,000 nuns); the "90,000+ Edo temples" and the Shaolin / Daxiangguo counts were found in no page read (adeac.jp Sapporo library page delivered no body; ja.wikipedia 寺請制度 gives only 6,000 Jōdo temples in 1696) - leftover `daxiangguo-enwiki` (READ: "several thousand" monks - more than the 1,000+ recorded)
...
**Sources:** `fushimi-inari-senbon` (READ: ~10,000 on the mountain, ~800 in the Senbon Torii, donated from Edo into Meiji). The "~600 in Meiji" figure was not on the NDL reference page read; the 1 / 2-3 / dozens distribution is assembled from normative statements, as the entry says - no census exists
...
**Sources:** `fushimi-inari-senbon` (READ: ~800 gates); the ~400 m run length, Kasuga's 1.3 km (klook, SUMMARY-ONLY; 1.2 km elsewhere) and Meiji Jingu's ~10-minute walk (tourism snippets) are SUMMARY-ONLY; `meiji-jingu-jawiki` counts 8 torii, so "three" there is the ranked set (一・二・三の鳥居), not the total - leftover for the distances
...
**Sources:** `louzeyuan-zhwiki` (READ: 1104, high barren ground, fenced, numbered plots, continued through Ming), `toribeno-jawiki` (READ: no defined boundary). Adashino, Rendaino and Osaka's seven graveyards (大坂七墓 page 404) not re-read - leftover `adashino-jawiki` (READ: 風葬 ground, later cremation; 蓮台野 has no article; 大坂七墓 404)
