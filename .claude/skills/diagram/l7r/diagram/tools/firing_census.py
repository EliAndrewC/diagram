...
def render(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "`FIRES` = the current implementation makes it fail. `FIRES-HAND-ONLY` = only a hand-era manifest or a",
        "hand-BUILT test dict does - proven teeth, unproven that the engine still trips it. **Neither is a**",
        "**deletion candidate**: the GM's ruling of 2026-08-30 is that hand-era-only evidence is a",
        "classification, not a criterion, so both go to the case-by-case ledger. Only a `NEVER-FIRES` row is a",
        "candidate, and it takes the placer read first - the census produces a candidate, not a ruling",
        "(feature 158), and that read deletes it only if it is DEAD (a name no scale can emit, a tier no",
        "generator can produce). Anything else joins the ledger.",
        "",
