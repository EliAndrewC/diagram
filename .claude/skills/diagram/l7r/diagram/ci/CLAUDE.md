...
| [`delta.py`](delta.py) | the Delta: what OUR commits changed since the merge base with `origin/main` (R1), and `is_engine()` - **the one list** of engine paths (FR-008): `l7r/**/*.py`, `pool/*.gen.py`, `pool/*.json` - the code the tests exercise, and nothing that merely shapes how the gate runs (GM 2026-08-25); **not `tests/`** since feature 132 FR-024 (the GM's ruling: a tests-only change skips the build and the local gate), and **not this `ci/` package** since FR-025 (the GM: *"isn't it actually test code?"* - a ci-only change is DIRECT and gets `make quick`); `tests/tooling/ci/test_delta.py` pins every path kind. Also `engine_key()`, the verification key |
...
| [`selection.py`](selection.py) | the pytest plugin the gate loads (`-p l7r.diagram.ci.selection`): switches the coverage context to `fixture:<name>` around every fixture setup, deselects what the plan says (identically on every xdist worker), writes the run's result and the fixture dependency graph from worker `gw0` |
