...
| `pondstock.py` | STAGE 6b (feature 139 A3/A4) - `stage_pond_stock`: a dike-pond hamlet's pig sties and duck pens on the ponds nearest the houses (`STY_SHARE` / `PEN_SHARE` are GUESS bands; the glyphs live in `settlement/farm_fixtures.py` `PondStockMixin`) |
