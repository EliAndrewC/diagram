...
| `ways/` | STAGES 4b and 5b - the lanes, the track and what makes a path legal. a PACKAGE with its own [`CLAUDE.md`](ways/CLAUDE.md) index since feature 173. Read that first, then load one of: `geom.py`, `checks.py`, `clearance.py`, `route.py`, `fabric.py`, `sweeps.py`, `touch.py`, `smooth.py`, `serve.py`, `web.py`, `track.py` - eleven LAYERS, bottom-up, so every reference points backwards. **The two stages and the order between them are still the whole design**; `web.py` and `track.py` hold them |
| `homesteads/` | STAGES 5-6 - the houses and what stands among them. a PACKAGE with its own [`CLAUDE.md`](homesteads/CLAUDE.md) index since feature 173. Read that first, then load one of: `seats.py` (`front_row`, `lane_frontage`, `cluster_aspect`), `bamboo.py` (the household strip), `fixtures.py` (`farmstead_fixtures` - the per-farmstead privy / heap / bath / coop / stack / hokora / persimmon placer), `wells.py` (`well_target`, `place_wells`), `stages.py` (`stage_homesteads`, `stage_appurtenances`) |
