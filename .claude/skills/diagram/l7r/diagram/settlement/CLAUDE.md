...
| `_geom/` | the geometry/spatial subsystem - a PACKAGE with its own [`CLAUDE.md`](_geom/CLAUDE.md) index since feature 117. Read that first, then load one of: `base.py` (the `Pt`/`Poly`/`Manifest` aliases, the import-time main-tree guard, the palette), `primitives.py` (point/segment/ring math), `overlap.py` (corner rings + the collision/gap/region predicates), `indexes.py` (the `boxed_*` prefilters, `PointGrid`, `Indexed`), `seatmemo.py` (`SeatMemo`), `labels.py` (the caption standoff ladder, the two caption sizes, tilt/quad/AABB), `ways.py` (the traveled ways, the kido bar, the crossing constants), `walls.py` (walls, ward closure, torii-vs-wall clearance), `extents.py` (a recorded feature's drawn extent), `curves.py` (fillets, smoothing, organic jitter), `village.py` (`village_population`, `BUNDLE_PITCH_FT`) |
...
| `water_ways/` | focal features, water and its clipping, lanes/streets/kido/wards/quarters - a PACKAGE with its own [`CLAUDE.md`](water_ways/CLAUDE.md) index since feature 173. Read that first, then load one of: `_helpers.py` (`junction_floor`, `fan_rival`), `focal.py`, `water.py`, `clipping.py`, `lanes.py`, `kido.py`, `wards.py` |
| `town_ways.py` | the TOWN-tier ways and focal features - `market`, `ancestral_hall`, `water_mouth`, `alley` (feature 145: moved out of `water_ways.py`, which every hamlet executes, so the module-level coverage floor judges only what a hamlet draws; `TownWaysMixin`) |
...
| `homestead_parts/` | threshing yards, gardens, groves, stands and keepouts - a PACKAGE with its own [`CLAUDE.md`](homestead_parts/CLAUDE.md) index since feature 173. Read that first, then load one of: `_helpers.py`, `yards.py`, `gardens.py`, `groves.py`, `stands.py` (`bamboo_stand`, `village_grove`), `keepouts.py`, `farmstead.py` (the three helpers feature 120 moved here) |
| `land/` | the land-surface subsystem - a PACKAGE with its own [`CLAUDE.md`](land/CLAUDE.md) index since feature 120, the LAST file in this package to be split. Another residue bucket rather than a chain, so its four modules are four unrelated subjects: read that index first, then load one of `dikes.py` (the polder perimeter dike + the dike-top village on its crest), `wet.py` (marsh and its four roles, `toe_band`, `trim_off_marsh`, and the module-level `surface_water_dist`), `cover.py` (the `commons` scatter, the `hinterland` composer, and the swept verge `_clear_ground`/`reserve_clearing` reserves), `nearring.py` (near-ring dry cropland and near-ring paddy) |
...
| `structures/urban_fixtures.py`, `city/crop.py`, `city/knobs.py`, `shrines_wells/forest.py` | the other town/city members moved off the hamlet path's modules by feature 145: the theater stage and drum tower (`UrbanFixturesMixin`), `crop_city` (`CityCropMixin`), `machi_mouths` / `moat_swept_tap`, the canvas-filling `forest` (`ForestMixin`); the Imperial road's caption became `GroundMixin._finish_road_label`, called from `finish()`. **The rule**: a function only another tier runs does not live in a module a hamlet roll executes - `make hamlet-floor` lists those modules and the FULL run holds them at 100% |
