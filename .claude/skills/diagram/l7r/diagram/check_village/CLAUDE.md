...
| `segments_07c_moats_drains_and_edges.py` | segs 0465-0512: moat junctions, in-wall drains, map-edge rules, halls on lanes, pond feeds; plus the three hand-added WAY-SERVICE rules, which belong together: 0607 `lanes_reach_something` (a lane must serve something), 0610 `farmhouses_reach_a_way` (its converse - and every house must be served, by the CONNECTED network), and 0611 `lane_ends_front_different_houses` (a farmhouse discharges one lane end's obligation, not three) |
...
| `segments_08d_kosatsuba_and_paddy_basins.py` | segs 0544-0608: manor gates, kosatsuba, punishment spots, paddy plot seams and basins; hand-added 0595 (supply-bank bunds), 0600 (comb floor ends at the collector), 0608 (basins worth their bund - the paddy size floor; peers took 0606 and 0607 the same day, so this one moved up twice) |
