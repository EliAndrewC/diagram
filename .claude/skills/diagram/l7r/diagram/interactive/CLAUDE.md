...
| `glossary.py` | you are adding a term the explanations use, or a definition reads wrong - every occurrence of a term in a modal is a hover tooltip; a test proves each term is used and each explanation's terms are defined. **It is the research record's glossary too** (feature 209, GM 2026-09-07: the same tooltip rules on the research pages): `record_glossary_js()` is the derived asset `research/assets/glossary.js`, written by `make glossary`, that `research/assets/record.js` wraps over every page's visible text; a term may live in the record alone, and `tests/interactive/test_record_format.py` proves the asset is in sync and every term used somewhere |
| `citations.py` | the record's CITATIONS PAGES (feature 211, GM 2026-09-07): the notes of `research/<name>.html` live on `research/citations/<name>.html`, and this module derives what the two pages share - the page's script `citations/<name>.js` (`window.RECORD_CITATIONS`, what `record.js` shows on hover) and the works section at the top of the citations page (each cited work's citation line and its two registry write-ups, in order of first citation, between markers). `make citations` (`tools/citations_asset.py`) writes both; `tests/interactive/test_citations.py` fails while a committed one differs or a cited key has no write-up. Look here when a hover shows nothing, a works list is stale, or a new key is refused |
| `sources.py` | a modal's references look wrong - they are READ FROM THE RECORD at page-write time: `research_questions()` resolves the class's `entry` to the research sections it names and builds each one's GitHub anchor (`RESEARCH_URL`, `github_anchor`, `question_text`); `research_sources()` / `registry()` read the `**Sources:**` keys and `SOURCES.html`, which the record tests still prove complete though the page no longer shows them; since feature 211 it also holds the ONE body of feature 190's link classifier (`citation_lines`, `not_read`, `link_target` - where a key links) and `registry_entries()` (an entry's citation line and its two write-ups), because `make citations` needs them and a tool does not import from tests/ |
