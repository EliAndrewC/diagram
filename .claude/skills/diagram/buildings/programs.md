...
- **The approach runs from the arch to the hall**, and the sanctuary stands BEHIND the hall on the same axis: arch, then hall, then sanctuary, each marked by id (`approach`, `arch`, `hall`, `sanctuary`). The arch stands where the way enters the precinct - it straddles the approach at the fence line, the one built thing a way runs under. **The arch count follows the map** (the setting's 1, 3 or 7 - `research/religion-and-death.html`, torii are votive donations): the innermost arch stands at the fence line, and any further arches stand on the approach OUTSIDE the fence, where the map puts them, with the approach drawn through all of them. Every arch is marked `id="arch"`, the innermost first. Checked: `sanctuary_on_axis`, `arch_on_approach` (the innermost arch), `matches_map` (every arch against the map's).
...
- **The grove surrounds the precinct**; the hall, the arch and the approach sit in a cleared opening, not under the canopy. **The burial ground lies beside the precinct** where no map says otherwise (a sheet on a map follows the map - knob 4), outside the fence, in the shrine's yard - the village rule on the village maps.
...
3. **Dedication and its furniture**: the Fortune the shrine serves and what that puts on the sheet - the instance particular, designed with the GM. The default is Inari, the rice Fortune, the ordinary rural dedication (GM 2026-09-20); the exemplar's is Bishamon, the Fortune of Strength - Hoshigaoka's shrine is the country monk Otsuki's, on the GM's word of the same day. What a dedication puts on the sheet: at a Bishamon shrine the guardian pair, where the wealth knob draws one, is tigers in place of lion-dogs (attested at named temples, a guess at village scale; the record's "What the dedication adds" on [`research/religion-and-death.html`](../research/religion-and-death.html)); at average wealth nothing is drawn beyond the name at the sanctuary.
4. **Grove and burial-ground side**: by the map when the shrine stands on one - the sheet declares it (`**On map**:`) and draws a grove or a burial ground only where the map has one inside the frame, on the map's side (both are site items in the declaration; the GM, 2026-09-20: the diagram view matches what the larger map shows) - else by the site as the program describes it.
