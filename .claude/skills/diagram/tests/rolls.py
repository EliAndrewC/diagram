...
doing only what is strictly necessary in order to reach one hundred percent code coverage"*): ZERO rows since feature
219 - the gate rolls no map of its own (GUARD_EDIT_OK: feature 219 - the immune requirement retired by the GM, Polder 12's
lines unit tests); its coverage is the five shipped generators and unit tests. A row added here is judged by 217's rule.
...
maps (the sweep rolls them cold, serves them warm; `tests/gate/_pool.py`), so the reference's one roll WAS the
immune experiment's perturbed one (retired in 219 - GUARD_EDIT_OK: feature 219); the re-roll loop runs on stand-in stages; the fan-out's pool child runs a
...
# actually okay with an upstream change in the number of random draws moving a map"* - the test, `rollcache.extra_draws`
