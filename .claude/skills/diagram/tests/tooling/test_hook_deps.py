...
def test_a_dependency_reached_only_through_another_helper_still_counts() -> None:
    ...
    `readme-hooks.sh` names `_guardlog.sh` and never names `_hookmatch.py`. But `_guardlog.sh`'s
    `escape_or_refuse` calls `_hookmatch.py` (feature 170), so a change to the matcher CAN break the
    readme suite. A direct-reference derivation would skip it and report green.
    """
    deps = hookdeps.deps_for("readme-hooks.sh")
    assert "_guardlog.sh" in deps, "the direct reference"
    assert "_hookmatch.py" in deps, (
...
def test_the_helpers_that_churn_are_NOT_narrowed_and_that_is_correct() -> None:
    ...
    `_hookmatch.py` and `_guardlog.sh` are genuinely used by nearly every guard, so their derived sets
    stay near 21. A future change that made these numbers SMALL would almost certainly be a derivation
    that had stopped following the graph - which is why this asserts a floor rather than a ceiling.
    """
    guards = sorted(p.name for p in SCRIPTS.glob("*-hooks.sh") if not p.name.startswith("test-"))
    matcher = [g for g in guards if "_hookmatch.py" in hookdeps.deps_for(g)]
...
def test_the_key_changes_when_a_dependency_changes(tmp_path, monkeypatch) -> None:
    ...
    def fake(name: str) -> str:
        return real(name) + ("# changed" if name == "_hookmatch.py" else "")
