...
figs_is_check5 = _f5mod.is_check5


def _lint14(*args, **kwargs) -> list[str]:
    """Checks 1 to 4 alone. These fixtures are unkeyed by design; check 5 is proven in its own tests below,
    and its messages are set aside by what they say (`_spec_figures.CHECK5_MARKS`), not by renumbering the
    fixture out of its reach - which is how a feature-number cutoff once hid in this file's history."""
    return [x for x in lint.lint(*args, **kwargs) if not figs_is_check5(x)]
...
def test_the_correct_form_is_quiet(tmp_path: pathlib.Path) -> None:
    assert _lint14(_feature(tmp_path)) == []
...
def test_check_1_a_measured_figure_with_no_pointer(tmp_path: pathlib.Path) -> None:
    ...
    assert any("no pointer" in x and "91 min" in x for x in _lint14(d))
...
def test_check_2_a_withdrawn_figure_still_standing(tmp_path: pathlib.Path) -> None:
    ...
    assert any("withdrawn text still standing" in x for x in _lint14(d))
...
def test_check_2_refuses_a_marker_too_short_to_be_safe(tmp_path: pathlib.Path) -> None:
    ...
    assert any("twelve characters" in x for x in _lint14(d))
...
def test_check_2_reaches_the_whole_tree(tmp_path: pathlib.Path) -> None:
    ...
    got = _lint14(d, tree_root=tmp_path)
...
def test_check_3_an_fr_no_criterion_names(tmp_path: pathlib.Path) -> None:
    ...
    assert any("FR-001a is named by no success criterion" in x for x in _lint14(d))
...
def test_check_3_a_criterion_that_names_nothing(tmp_path: pathlib.Path) -> None:
    ...
    assert any("names no FR" in x for x in _lint14(d))
...
def test_check_4_a_task_citing_an_id_the_spec_lost(tmp_path: pathlib.Path) -> None:
    ...
    assert any("FR-404" in x for x in _lint14(d))
...
def test_check_5_reaches_an_old_feature(tmp_path: pathlib.Path) -> None:
    """No feature-number cutoff: the motivating case was feature 236's amendment, below 239."""
    body = "## Summary\n\nIt costs 144 ms per command.\n"
    assert figs.check_measured_figures(_measured(tmp_path, body, _ENTRY, number=236)) != []


def test_check_5_reaches_a_draft_with_no_tasks(tmp_path: pathlib.Path) -> None:
    """No tasks.md condition: spec review happens before tasks exist, which is where figures reach a reviewer."""
    d = _measured(tmp_path, "## Summary\n\nIt costs 144 ms per command.\n", _ENTRY, number=240)
    (d / "tasks.md").unlink()
    assert figs.check_measured_figures(d) != []


def test_the_number_claim_still_passes(tmp_path: pathlib.Path) -> None:
    """The claim pushes an EMPTY directory - which is why the tasks.md condition protected nothing there."""
    d = tmp_path / "specs" / "241-just-claimed"
    d.mkdir(parents=True)
    assert lint.lint(d) == [] and figs.check_measured_figures(d) == []


def test_the_spec_template_does_not_trip_check_5(tmp_path: pathlib.Path) -> None:
    """A draft still carrying template text must not fail: its example figures are NAMED, in backtick spans."""
    d = tmp_path / "specs" / "241-a-draft"
    d.mkdir(parents=True)
    (d / "spec.md").write_text((REPO / ".specify" / "templates" / "spec-template.md").read_text())
    assert figs.check_measured_figures(d) == []
