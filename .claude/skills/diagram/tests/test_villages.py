...
def test_every_pool_gen_is_classified():
    ...
    # Every declared Mode A tier that exists holds only compounds, and every compound sits in a declared tier.
    from l7r.diagram.buildings.types import tiers as building_tiers

    off_tier = [b.stem for b in bundles if (b.kind == "compound") != (b.tier in building_tiers())]
    assert not off_tier, f"a compound outside a declared Mode A tier, or a non-compound inside one: {off_tier}"
