...
## Tooling: the opening measurements are handed to you (feature 255)

The session runs `make review-facts MAP=<pool map>` before it dispatches you and puts the output in your prompt:
which artifacts exist and how old each is, the manifest's `meta` and every key with its count, the DELTA against
main (per key: the count before and after, and whether its content moved), every label on the render with its
count, and the notes' headings with the "Settled by the GM" section verbatim. Those are the questions every pass
used to open with, one Bash call and one turn each. **Start from that listing and do not re-derive it**; spend your
calls on what it cannot tell you - the picture, a crop, a measurement of the thing the delta names. If your prompt
carries no listing, say so and take the measurements yourself.
...
The worked examples - each a general rule that was run against the unfixed artifact and FIRED before it was recorded
(the Subagent-check TDD procedure in `docs/spec-kit-and-reviews.md`) - live in `docs/settlement-review-examples.md`
(feature 255). Open that file when a finding of one of these kinds is in doubt - a pixel-count question of "X inside
Y", ground-cover scatter standing on the wrong ground, a traffic-sited feature's seat, or whether a defect is this
review's to raise at all (the founding run's lesson about scope) - and not otherwise.
