...
## FIRST STAGE - the explanation's counterfactual, before anything else (feature 240, GM 2026-09-13)

An attribution is only as good as the run that removed the cause. Feature 230's first explanation said seed
4's web growth was "the map, not the code", because none of the feature's new passes appeared in a
cumulative-time profile; the audit then forced the suspected rule to return True and measured the web stage at
4.70 s live, 2.56 s forced, against a 1.34 s baseline - most of the growth WAS the code
(`specs/240-verified-before-reviewed/research.md` R2). A profile says where the time went, never why. So
`make perf-explain` now records its evidence as a structured field, and you read it BEFORE the report, the
diff or any profile:

- **`control`** names a key in some `specs/*/measurements.json` - a recorded run of the same subject with the
  attributed cause removed or disabled; `control_record` carries the record as it stood. If the key names no
  record there now, return **NOT-REVIEWABLE** naming the key, record nothing, and stop: an explanation whose
  counterfactual cannot be found has not been tested. Otherwise, check the record is what it claims - the same
  seed and stage, the cause actually removed - before you weigh the explanation against it.
- **`unverified`** means the session did not test its attribution, and says why. Then **run the counterfactual
  yourself, before any other work** - disable or force the suspected cause at the end commit, in a detached
  worktree, and time the stage it names (`make perf-profile SEED=<n> STAGE=<stage>` on both). These are the
  GM's *"numbers generated by the reviewer"*. A detached worktree carries no gitignored artifacts - no roll cache, no rendered pool - so time BOTH legs inside worktrees, never one leg there and one in the clone; and a profile that will run past two minutes is given a longer timeout, never waited on in a sleep loop (the no-poll guard refuses a busy-wait). If you cannot run it, return **NOT-REVIEWABLE** saying what
  stopped you; an untested attribution never reaches a confirmation or an audit.

A NOT-REVIEWABLE return writes no review record. The session records the control and explains again.
